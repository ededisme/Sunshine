...
$aColumns = array("invoice_details.id",
    "invoice_details.created",
    "invoices.invoice_code",
    //"(SELECT SECTION.name FROM sections SECTION WHERE SECTION.id = services.section_id)",
    "sections.name",
    "services.name",
    "invoice_details.qty",
...
/* DB table to use */
$sTable = "invoice_details INNER JOIN invoices ON invoices.id = invoice_details.invoice_id INNER JOIN services ON services.id = invoice_details.service_id INNER JOIN sections on sections.id = services.section_id";
...
/* Customize condition */
$condition = 'invoice_details.is_active = 1 AND sections.is_active = 1 AND services.is_active = 1 AND invoice_details.type = 2';
if ($data[1] != '') {
...
while ($aRow = mysql_fetch_array($rResult)) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        if ($i == 0) {
            ...
            $row[] = ++$index;
        } else if ($aColumns[$i] == 'invoice_details.created') {
