...
/**
 ...
 2.0.1
 - Add support for browsers with cookies disabled (requires php5, mysql) maps users to md5 hashed ip addresses and md5 hashed codes for security
...
class Securimage {
    ...
    /**
     * Use an mysql database for storing codes as a backup to sessions.<br />
    ...
    function Securimage() {
        ...
        $this->mysql_database = mysql_connect("localhost","root","");
        $this->use_mysql_db   = true;
        $this->mysql_handle = true;

        @mysql_select_db("emr_0813_ponleukpc", $this->mysql_database);
    }
...
//        $_SESSION['securimage_code_ctime'] = time();
     ...
     */
    function validate() {
        // retrieve code from session, if no code exists check mysql database if supported.
    ...
    function getCode() {
        if (isset($_SESSION['securimage_code_value']) && !empty($_SESSION['securimage_code_value'])) {
            ...
            if ($this->mysql_handle == false) $this->openDatabase();

            return $this->getCodeFromDatabase(); // attempt to get from database, returns empty string if mysql is not available or disabled
        }
    }
    ...
    function addSignature() {
        if ($this->use_gd_font) {
            ...
            imagettftext($this->im, 7, 0, $x, $y, $this->gdsignaturecolor, $this->signature_font, $this->image_signature);
        }
    }
     ...
     */
    function getIPHash() {
        return strtolower(md5($_SERVER['REMOTE_ADDR'].$_SERVER['HTTP_USER_AGENT'].gethostbyaddr($_SERVER['REMOTE_ADDR'])));
    }
    ...
    function openDatabase() {
        $this->mysql_handle = false;

        if ($this->use_mysql_db && function_exists('mysql_open')) {
            $this->mysql_handle = mysql_open($this->mysql_database, 0666, $error);

            if ($this->mysql_handle !== false) {
                $res = mysql_query($this->mysql_handle, "PRAGMA table_info(codes)");
                if (mysql_num_rows($res) == 0) {
                    mysql_query($this->mysql_handle, "CREATE TABLE codes (iphash VARCHAR(32) PRIMARY KEY, code VARCHAR(32) NOT NULL, created INTEGER)");
                }
            }

            return $this->mysql_handle != false;
        }
    ...
    /**
     * Save captcha code to mysql database
     *
     * @access private
     * @since 2.0.1
    ...
    function saveCodeToDatabase() {
        ...
        $ip = $this->getIPHash();
        $time = time();
        $code = $this->code; // hash code for security - if cookies are disabled the session still exists at this point
        $this->clearCodeFromDatabase();
        $success = mysql_query("INSERT INTO codes(iphash, code, created) VALUES('$ip', '$code', $time)");
        return $success !== false;
    }

    /**
     * Get stored captcha code from mysql database based on ip address hash
     *
     * @access private
     * @since 2.0.1
    ...
    function getCodeFromDatabase() {
        $code = '';
        $ip = $this->getIPHash();
        $res = mysql_query("SELECT * FROM codes WHERE iphash = '$ip'");
    ...
    function clearCodeFromDatabase() {
        $ip = $this->getIPHash();
        mysql_query("DELETE FROM codes WHERE iphash = '$ip'");
    }
    ...
    function purgeOldCodesFromDatabase() {
        if ($this->use_mysql_db && $this->mysql_handle !== false) {
            $now   = time();
            $limit = (!is_numeric($this->expiry_time) || $this->expiry_time < 1) ? 86400 : $this->expiry_time;

            mysql_query($this->mysql_handle, "DELETE FROM codes WHERE $now - created > $limit");
        }
    }
