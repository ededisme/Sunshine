...
<script type="text/javascript">
    $(document).ready(function(){
        // Prevent Key Enter
        preventKeyEnter();
        $("#LocationGroupTypeEditForm").validationEngine('attach', {
            isOverflown: true,
            overflownDIV: ".ui-tabs-panel"
        });
        $("#LocationGroupTypeEditForm").ajaxForm({
            ...
            success: function(result) {
                ...
                if(result != '<?php echo MESSAGE_DATA_HAS_BEEN_SAVED; ?>' && result != '<?php echo MESSAGE_DATA_COULD_NOT_BE_SAVED; ?>' && result != '<?php echo MESSAGE_DATA_ALREADY_EXISTS_IN_THE_SYSTEM; ?>'){
                    createSysAct('Warehouse Type', 'Edit', 2, result);
                    $("#dialog").html('<p><span class="ui-icon ui-icon-info" style="float:left; margin:0 7px 20px 0;"></span><?php echo MESSAGE_PROBLEM; ?></p>');
                }else {
                    createSysAct('Warehouse Type', 'Edit', 1, '');
                    // alert message
...
<br />
<?php 
echo $this->Form->create('LocationGroupType');
echo $this->Form->input('id');
echo $this->Form->hidden('sys_code');
?>
<input type="hidden" name="data[LocationGroupType][allow_negative_stock]" id="LocationGroupTypeAllowNegativeStock" value="<?php echo $this->data['LocationGroupType']['allow_negative_stock']; ?>" />
<input type="hidden" name="data[LocationGroupType][stock_tranfer_confirm]" id="LocationGroupTypeStockTransferConfirm" value="<?php echo $this->data['LocationGroupType']['stock_tranfer_confirm']; ?>" />
<fieldset style=" height: 238px;">
    ...
    <table>
        ...
        <tr>
            <td></td>
            <td>
                <input type="checkbox" <?php if($this->data['LocationGroupType']['allow_negative_stock'] == 1){ ?>checked=""<?php } ?> id="CheckLocationGroupTypeAllowNegativeStock" /><label for="CheckLocationGroupTypeAllowNegativeStock"><?php echo TABLE_ALLOW_NEGATIVE_STOCK; ?> (Sales)</label>
            </td>
        </tr>
    </table>
</fieldset>
