 ...
 */
$aColumns = array('cus.id', 'cus.customer_code', 'cus.name_kh', 'cus.name', 'cus.sex', 'cus.main_number', '(SELECT net_days FROM payment_terms WHERE id=payment_term_id)');
...
while ($aRow = mysql_fetch_array($rResult)) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        if ($i == 0) {
            ...
            $index ++;
            $row[] = '<input type="radio" value="'.$aRow[0].'" name="chkCustomer" rel="'.$aRow[2].' ('.$aRow[3].')" net_days="'.$aRow[6].'" />' ;
        } else if ($i == 6) {
