...
/* Customize condition */
$condition = "gl.is_active = 1 AND is_sys = 0 AND branch_id IN (SELECT branch_id FROM user_branches WHERE user_id = ".$user['User']['id'].")";
...
while ($aRow = mysql_fetch_array($rResult)) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        if ($i == 0) {
            ...
            if ($aRow[0] == $tmpId) {
                ...
                $row[] = '<a href="" class="btnChangeStatusJournal" rel="' . $aRow[0] . '" name="' . $aRow[2] . '" status="' . $aRow[$i] . '"><img alt="' . ($aRow[$i] == 1 ? TABLE_ACTIVE : TABLE_INACTIVE) . '" onmouseover="Tip(\'' . ($aRow[$i] == 1 ? TABLE_ACTIVE : TABLE_INACTIVE) . '\')" src="' . $this->webroot . 'img/button/' . ($aRow[$i] == 1 ? 'active' : 'inactive') . '.png" /></a>';
            }
    ...
    if ($aRow[0] == $tmpId) {
        ...
        $row[] =
                ($allowViewAll ? '<a href="" class="btnViewJournalEntryAll" rel="' . $aRow[0] . '" name="' . $aRow[1] . '"><img alt="View" onmouseover="Tip(\'' . ACTION_VIEW . '\')" src="' . $this->webroot . 'img/button/view.png" /></a> ' : '') .
                ($allowViewAll ? '<a href="" class="btnPrintJournalEntryAll" rel="' . $aRow[0] . '" type="' . $aRow[5] . '"><img alt="Print" onmouseover="Tip(\'' . ACTION_PRINT . '\')" src="' . $this->webroot . 'img/button/printer.png" /></a> ' : '') .
                ($allowViewAll ? '<a href="" class="btnNoteJournalEntryAll" rel="' . $aRow[0] . '" note="' . str_replace('"', "{dblquote}", trim($aRow[12])) . '"><img alt="Note" onmouseover="Tip(\'' . TABLE_NOTE . '\')" src="' . $this->webroot . 'img/button/note.png" /></a> ' : '') .
                ($allowEditAll ? '<a href="" class="btnEditJournalEntryAll" rel="' . $aRow[0] . '" name="' . $aRow[1] . '"><img alt="Edit" onmouseover="Tip(\'' . ACTION_EDIT . '\')" src="' . $this->webroot . 'img/button/edit.png" /></a> ' : '') .
                ($allowDeleteAll ? '<a href="" class="btnDeleteJournalEntryAll" rel="' . $aRow[0] . '" name="' . $aRow[1] . '"><img alt="Delete" onmouseover="Tip(\'' . ACTION_DELETE . '\')" src="' . $this->webroot . 'img/button/delete.png" /></a>' : '');
    }
    $output['aaData'][] = $row;
    $tmpId = $aRow[0];
}
