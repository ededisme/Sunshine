...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $("#GeneralLedgerEditAllForm").validationEngine('attach', {
            isOverflown: true,
            overflownDIV: ".ui-tabs-panel"
        });
        $("#GeneralLedgerEditAllForm").ajaxForm({
            beforeSerialize: function($form, options) {
                ...
                if(notEqual){
                    ...
                    var confirmSave = $("#GeneralLedgerEditAllConfirmSave").val();
                    // Check Confirm Save
                    if(confirmSave == 0){
                        confirmSaveEntryEditAll();
                        $("#GeneralLedgerDate").datepicker("option", "dateFormat", "dd/mm/yy");
                        $("button[type=submit]", $form).removeAttr('disabled');
                        return false;
                    }
            ...
            success: function(result) {
                ...
                var rightPanel=$("#GeneralLedgerEditAllForm").parent();
                var leftPanel=rightPanel.parent().find(".leftPanel");
                rightPanel.hide();rightPanel.html("");
                leftPanel.show("slide", { direction: "left" }, 500);
                ...
                if(result != '<?php echo MESSAGE_DATA_HAS_BEEN_SAVED; ?>' && result != '<?php echo MESSAGE_DATA_COULD_NOT_BE_SAVED; ?>'){
                    createSysAct('Journal Entry Supervisor', 'Edit', 2, result);
                    $("#dialog").html('<p><span class="ui-icon ui-icon-info" style="float:left; margin:0 7px 20px 0;"></span><?php echo MESSAGE_PROBLEM; ?></p>');
                }else {
                    createSysAct('Journal Entry Supervisor', 'Edit', 1, '');
    ...
    function confirmSaveEntryEditAll(){
        ...
        $("#dialog").dialog({
            ...
            buttons: {
                ...
                '<?php echo ACTION_YES; ?>': function() {
                    $("#GeneralLedgerEditAllConfirmSave").val(1);
                    $("#GeneralLedgerEditAllForm").submit();
                    $(this).dialog("close");
                }
            }
        });
...
<input type="hidden" id="GeneralLedgerEditAllConfirmSave" value="0" />
<fieldset>
...
<table id="tblGL" class="table" cellspacing="0">
    ...
    while($dataGeneralLedgerDetail=mysql_fetch_array($queryGeneralLedgerDetail)){
        ...
        if($dataGeneralLedgerDetail['customer_id'] != ''){
            ...
            $otherName = $sqlOther[0];
            $displayDiv   = 1;
            $displayOth   = "";
        }
    ?>
    <tr class="tblJournalSuperRow">
        <td class="first">
        ...
        <td>
            <div class="inputContainer">
                <input type="text" id="debit<?php echo $index; ?>" name="debit[]" class="debit validate[required] float" value="<?php echo number_format($dataGeneralLedgerDetail['debit'], 2); ?>" />
            </div>
        </td>
        <td>
            <div class="inputContainer">
                <input type="text" id="credit<?php echo $index; ?>" name="credit[]" class="credit validate[required] float" value="<?php echo number_format($dataGeneralLedgerDetail['credit'], 2); ?>" />
            </div>
        </td>
        <td>
            <div class="inputContainer" style="width: 100%;">
                <input type="text" id="memo<?php echo $index; ?>" name="memo[]" class="memo" value="<?php echo $dataGeneralLedgerDetail['memo']; ?>" onmouseover="Tip('<?php echo str_replace(array("'",'"',"\\"),array("&rsquo;","&quot;","&#92;"),$dataGeneralLedgerDetail['memo']); ?>')" style="width: 90%;" />
            </div>
        </td>
        <td>
            <div class="inputContainer">
