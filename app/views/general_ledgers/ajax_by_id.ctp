...
/* Customize condition */
$condition = "gl.is_approve=1 AND gl.is_active=1 AND gl.id=" . $general_ledger_id. " AND gld.company_id IN (SELECT company_id FROM user_companies WHERE user_id = ".$user['User']['id'].") AND gld.branch_id IN (SELECT branch_id FROM user_branches WHERE user_id = ".$user['User']['id'].")";
