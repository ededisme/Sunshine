...
$arrGeleralLedgerDetailIdListByCoAGroupId = array();
$queryGeleralLedgerDetailIdListByCoAGroupId = mysql_query('SELECT id FROM general_ledger_details WHERE chart_account_id IN (SELECT id FROM chart_accounts WHERE chart_account_group_id=' . $chart_account_group_id . ')');
while ($dataGeleralLedgerDetailIdListByCoAGroupId = mysql_fetch_array($queryGeleralLedgerDetailIdListByCoAGroupId)) {
    $arrGeleralLedgerDetailIdListByCoAGroupId[] = $dataGeleralLedgerDetailIdListByCoAGroupId['id'];
}
if ($companyId &&  $companyId != 'null') {
    $condition .= " AND gld.company_id='" . $companyId . "'";
}else{
...
$condition .= " AND gl.date <= '" . $dateAsOf . "' AND gld.id IN (" . implode(",", $arrGeleralLedgerDetailIdListByCoAGroupId) . ")";
...
while ($aRow = mysql_fetch_array($rResult)) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        if ($i == 0) {
            ...
            $queryGlTypeId = mysql_query("SELECT chart_account_type_id FROM chart_account_groups WHERE id=" . $chart_account_group_id);
