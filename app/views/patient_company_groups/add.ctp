...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        // Prevent Key Enter
        preventKeyEnter();
        $("#PatientCompanyGroupsAddForm").validationEngine();
        $("#PatientCompanyGroupsAddForm").ajaxForm({
            beforeSerialize: function($form, options) {
//                listbox_selectall('vendor_id', true);
            },
            beforeSubmit: function(arr, $form, options) {
                $(".txtSaveCompanyGroup").html("<?php echo ACTION_LOADING; ?>");
...
<br />
<?php
echo $this->Form->create('PatientCompanyGroups');
?>
<fieldset>
    <legend><?php __(MENU_COMPANY_NAME); ?></legend>
    <table>
        <tr>
            <td style="width: 25%;"><label for="PatientGroupParentId"><?php echo TABLE_SUB_OF_GROUP; ?>:</label></td>
            <td>
                <select id="PatientGroupParentId" name="data[PatientCompanyGroups][parent_id]">
                    ...
                    while($rowParent = mysql_fetch_array($sqlParent)){
                        ?>
                        <option com="<?php echo $rowParent['id']; ?>" value="<?php echo $rowParent['id']; ?>"><?php echo $rowParent['name']; ?></option>
                        <?php
                    }
                    ?>
                </select>
...
<div class="buttons">
    <button type="submit" class="positive btnSaveCompanyGroup">
        ...
        <span class="txtSaveCompanyGroup"><?php echo ACTION_SAVE; ?></span>
    </button>
</div>
<div style="clear: both;"></div>
