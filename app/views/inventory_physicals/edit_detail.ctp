...
<script type="text/javascript">
	...
	$("#OrderListInventoryPhysical").remove();
        ...
        $("#btnHideShowHeaderInventoryPhysical").click(function(){
            ...
            var InventoryPhysicalChartAccountId  = $("#InventoryPhysicalChartAccountId").val();
            var InventoryPhysicalLocationGroupId = $("#InventoryPhysicalLocationGroupId").val();
            
            if(InventoryPhysicalCompanyId == "" || InventoryPhysicalDate == "" || InventoryPhysicalChartAccountId == "" || InventoryPhysicalLocationGroupId == ""){
        ...
        // Search Product PUC
        $("#InventoryPhysicalPUC").keypress(function(e){
            if((e.which && e.which == 13) || e.keyCode == 13){
                if(searchAction == 0){
                    searchAction = 1;
                    searchProductInventoryPhysical(2, $(this).val());
                }
                return false;
            }
        });
        ...
        $("#InventoryPhysicalSKU, #InventoryPhysicalPUC").focus(function(){
    ...
    function searchProductInventoryPhysical(type, code){
        ...
        var date          = $("#InventoryPhysicalDate").val();
        var url           = '';
        $("#InventoryPhysicalSKU, #InventoryPhysicalPUC").val("");
    ...
    function addProductInventoryPhysical(result){
        ...
        if(checkExistingProductInventoryPhysical(result.Product.id, '0', '0000-00-00') == false){
            ...
            tr.find(".newQtyLabel").attr("id", "newQtyLabel_"+rowIndexInventoryPhysical);
                        ...
                        buttons: {
                            '<?php echo ACTION_OK; ?>': function() {
                                ...
                                $(".cloneInventoryPhysical").each(function(){
                                    qtyInventoryPhysicalDiff += parseFloat($(this).find(".floatQtyInventoryPhysicalUom").val().replace(/,/g,"")) * parseFloat($(this).find(".qty_uom_cycle").find("option:selected").attr("conversion"));
                                });
    ...
    function getTotalQtyOnHand(obj){
        ...
        var totalQtyInventoryPhysical = obj.closest("tr").find("input[name='new_qty[]']").val();
        $("#InventoryPhysicalDate").datepicker("option", "dateFormat", "yy-mm-dd");
        ...
        if(location != '' && productId != '' && adjDate != ''){
            ...
            $.ajax({
                ...
                data:   "product_id="+productId+"&inv_adj_id=<?php echo $InventoryPhysicalId; ?>",
                beforeSend: function(arr, $form, options) {
                    $(".loader").attr('src','<?php echo $this->webroot; ?>img/layout/spinner.gif');
...
<div class="inputContainer" style="width:100%; float: left;" id="searchFormInventoryPhysical">
    <table style="width: 100%;">
        <tr>
            ...
            <td>
                <img alt="Search" align="absmiddle" style="cursor: pointer; width: 22px; height: 22px;" class="searchProductInventoryPhysical" onmouseover="Tip('<?php echo GENERAL_SEARCH; ?>')" src="<?php echo $this->webroot . 'img/button/search.png'; ?>" />
            </td>
        </tr>
    </table>
</div>
...
<div id="bodyListInventoryPhysical">
    <table id="tblInventoryPhysical" class="table" cellspacing="0" style="padding: 0px; width:100%">
        <tr id="OrderListInventoryPhysical" class="tblInventoryPhysicalList" style="visibility: hidden;">
            ...
            <td style="width: 14%;">
                <div class="inputContainer" style="width:100%">
                    <input type="hidden" class="InventoryPhysicalQtyDifferenceHidden" name="qty_difference[]" value="0" />
        ...
        <?php
        $index = 0;
        $sqlDetail = mysql_query("SELECT * FROM inventory_physical_details WHERE inventory_physical_id = ".$InventoryPhysicalId);
        while($rowDetail=mysql_fetch_array($sqlDetail)){
            $sqlProduct = mysql_query("SELECT code, barcode, name, small_val_uom, (SELECT name FROM uoms WHERE id=price_uom_id) As uom_name, (SELECT abbr FROM uoms WHERE id=price_uom_id) As uom_abbr, price_uom_id, is_expired_date, is_lots FROM products WHERE id = ".$rowDetail['product_id']);
            $rowProduct = mysql_fetch_array($sqlProduct);
        ?>
        <tr class="tblInventoryPhysicalList">
            <td class="first" style="width: 3%;"><?php echo ++$index; ?></td>
            <td style="width: 10%;">
                <input type="text" class="PUCLabel" value="<?php echo $rowProduct['barcode']; ?>" readonly="" style="width: 95%; height: 25px;" />
            </td>
            <td style="width: 16%;">
                <div class="inputContainer" style="width:100%">
                    <input type="hidden" name="product_id[]" value="<?php echo $rowDetail['product_id']; ?>" class="product_id" />
                    <input type="text" style="width: 90%; height: 25px;" id="productname_<?php echo $index; ?>" value="<?php echo str_replace('"', '&quot;', $rowProduct['name']); ?>" class="product_name" readonly="readonly" />
                </div>
            </td>
            <td style="width: 9%; <?php if($rowSettingUomDetail[0] == 0){ ?>display: none;<?php } ?>">
                <div class="inputContainer" style="width:100%">
                    <?php
                    $lotRequire = '';
                    $lotDisplay = 'visibility: hidden;';
                    if($rowProduct['is_lots'] == 1){
                        $lotRequire = 'class="lots_number validate[required]"';
                        $lotDisplay = '';
                    }
                    ?>
                    <input type="text" name="lots_number[]" id="lots_number_<?php echo $index; ?>" value="<?php echo $rowDetail['lots_number']; ?>" style="width: 90%; height: 25px; <?php echo $lotDisplay; ?>" <?php echo $lotRequire; ?> />
                </div>
            </td>
            <td style="width: 10%;">
                <div class="inputContainer" style="width:100%">
                    <?php
                    $dateExp = $rowDetail['expired_date']!=""&&$rowDetail['expired_date']!="0000-00-00"?dateShort($rowDetail['expired_date']):"0000-00-00";
                    $expRequire = '';
                    $expDisplay = 'visibility: hidden;';
                    if($rowProduct['is_expired_date'] == 1){
                        $expRequire = 'class="expired_date validate[required]"';
                        $expDisplay = '';
                    }
                    ?>
                    <input type="text" name="expired_date[]" id="expired_date_<?php echo $index; ?>" value="<?php echo $dateExp; ?>" style="width: 90%; height: 25px; <?php echo $expDisplay; ?>" <?php echo $expRequire; ?> readonly="readonly" />
                </div>
            </td>
            <td style="width: 12%;">
                <div class="inputContainer" style="width:100%">
                    <select style="width: 90%; height: 25px;" name="location_id[]" id="location_id_<?php echo $index; ?>" class="location_id validate[required]">
                        ...
                        ?>
                        <option value="<?php echo $rowLoc['id']; ?>" <?php if($rowLoc['id']==$rowDetail['location_id']){ ?>selected="selected"<?php } ?>><?php echo $rowLoc['name']; ?></option>
                        <?php
                        }
                        $smallUomLabel = "";
            ...
            <td style="width: 10%;">
                <input type="hidden" class="uomMainId" value="<?php echo $rowProduct['price_uom_id']; ?>" />
                <input type="hidden" class="uomMainLabel" value="<?php echo $rowProduct['uom_abbr']; ?>" />
                <input type="hidden" class="smallUomInventoryPhysical" value="<?php echo $rowProduct['small_val_uom']; ?>" />
                <input type="hidden" class="smallUomLabelInventoryPhysical" value="<?php echo $smallUomLabel; ?>" />
                <span class="uom_label"><?php echo $rowProduct['uom_name']; ?></span>
            </td>
            <?php
            // Calculate Total Qty On Hand
            if($rowProduct['is_expired_date'] == 1){
                $sqlTotal = mysql_query("SELECT SUM((total_cycle + total_to_in + total_cm + total_pb + total_cus_consign_in) - (total_so + total_pos + total_pbc + total_to_out + total_cus_consign_out + total_order)) AS total_qty FROM `".$rowDetail['location_id']."_inventory_total_details` WHERE product_id = {$rowDetail['product_id']} AND lots_number = '{$rowDetail['lots_number']}' AND expired_date = '{$rowDetail['expired_date']}' AND date <= '".$InventoryPhysical['InventoryPhysical']['date']."' GROUP BY product_id, expired_date;");
            }else{
                $sqlTotal = mysql_query("SELECT SUM(total_qty - total_order) AS total_qty FROM `".$rowDetail['location_id']."_inventory_totals` WHERE product_id = {$rowDetail['product_id']} AND lots_number = '{$rowDetail['lots_number']}' AND expired_date = '{$rowDetail['expired_date']}'");
            }
            $totalOrder = 0;
            $sqlOrder = mysql_query("SELECT sum(sor.qty) as total_order FROM `stock_orders` as sor WHERE sor.inventory_physical_id = ".$InventoryPhysical['InventoryPhysical']['id']." AND sor.product_id = ".$rowDetail['product_id']." AND sor.location_group_id = ".$InventoryPhysical['InventoryPhysical']['location_group_id']." AND location_id = ".$rowDetail['location_id']." AND lots_number = '{$rowDetail['lots_number']}' AND expired_date = '{$rowDetail['expired_date']}' AND date <= '".$InventoryPhysical['InventoryPhysical']['date']."' GROUP BY sor.product_id");
            ...
            // Calculate New Qty & Diff Qty
            $totalDiffDisplay = isset($rowDetail['qty_diff']) ? $rowDetail['qty_diff'] : 0;
            $displayDiff = showTotalQty($totalDiffDisplay, $rowProduct['uom_abbr'], $rowProduct['small_val_uom'], $smallUomLabel);
            ?>
            <td style="width: 14%;">
                <div class="inputContainer" style="width:100%">
                    <input type="hidden" class="InventoryPhysicalQtyDifferenceHidden" name="qty_diff[]" value="<?php echo $totalDiffDisplay; ?>" />
                    <input type="text" class="InventoryPhysicalQtyDifference" id="InventoryPhysicalQtyDifference_<?php echo $index; ?>" value="<?php echo $displayDiff; ?>" readonly="readonly" style="width: 90%; height: 25px;" />
                </div>
            </td>
            <td style="width: 13%;">
                <div class="inputContainer" style="width:100%">
                    <input type="hidden" class="qtyInventoryPhysicalCurrent" name="total_qty[]" value="<?php echo $totalQty; ?>" />
                    <input type="text" class="qtyOnHandLabel" id="qtyOnHandLabel_<?php echo $index; ?>" value="<?php echo showTotalQty($totalQty, $rowProduct['uom_abbr'], $rowProduct['small_val_uom'], $smallUomLabel); ?>" readonly="readonly" style="width: 90%; height: 25px;" />
                </div>
            </td>
            <td style="width: 3%;">
                <img alt="Remove" src="<?php echo $this->webroot . 'img/button/cross.png'; ?>" class="btnRemoveInventoryPhysical" align="absmiddle" style="cursor: pointer;" onmouseover="Tip('Remove')" />
