...
$index = 1;
$query_followup = mysql_query("SELECT * FROM doctor_daignostics WHERE is_active=1 AND patient_consultation_id=" . $patientConsultationId." ORDER BY created ASC");
while ($data_followup = mysql_fetch_array($query_followup)) {
    ...
    <tr>
        ...
        <td style="width: 5%;border: none; font-weight: bold; text-align: right; font-size: 11px !important;background-color: rgb(204, 204, 204); color: rgb(0, 0, 0);">
            ...
            if($data_followup['created_by'] == $user['User']['id'] || $allowAdmin){
                echo '<a href="#" type="doctor_daignostic" class="btnEditDoctorDaignosticIPD" rel="' . $data_followup['id'] . '" ><img alt="Edit" onmouseover="Tip(\'' . ACTION_EDIT . '\')" src="' . $this->webroot . 'img/action/edit.png" /></a>';
            }
            ?>
        </td>
    </tr>
    <tr>
        <td style="border: none; padding-left: 5%; color: rgb(0, 0, 0);" colspan="3">
            <?php echo nl2br($data_followup['doctor_daignostic']);?>
        </td>
    </tr>
<?php } ?>