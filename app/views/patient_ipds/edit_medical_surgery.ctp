...
<script type="text/javascript">
    $(document).ready(function(){ 
        // Prevent Key Enter
        preventKeyEnter();        
        $("#PatientIpdEditMedicalSurgeryForm").validationEngine();
        $("#PatientIpdEditMedicalSurgeryForm").ajaxForm({
            dataType: 'json',
        ...
        // submit form and print patient ipd
        var i = 1;
        $("#submit").click(function(){
            
            if($("#PatientIpdEditMedicalSurgeryForm").validationEngine('validate')){                                    
                if(i>1){
                    return false;
                }else{
                ...
                i++;
                var btnQuotation=$("#<?php echo $tblName;?>").html();                
                $("#<?php echo $tblName;?>").html('<p style="text-align:center"><img alt="" src="<?php echo $this->webroot; ?>img/loading.gif" /></p>');
                $("#<?php echo $tblName;?>").dialog({
                    title: 'Saving',
                    resizable: false,
                    modal: true,
                    close: function() {
                        window.open("<?php echo $absolute_url . $this->params['controller']; ?>/medicalSurgery/","_self");
                    },
                    buttons: {
                        Ok: function() {
                            $( this ).dialog( "close" );
                ...
                var url = '<?php echo $absolute_url . $this->params['controller']; ?>/editMedicalSurgery';                                               
                var post = $('#PatientIpdEditMedicalSurgeryForm').serialize();

                $.post(url,post,function(queueId){                           
                    $("#<?php echo $tblName;?>").html(btnQuotation);
                    $("#<?php echo $tblName;?>").dialog({
                    ...
                    $("#quotationPatientIpd").load("<?php echo $absolute_url . $this->params['controller']; ?>/printPatientMedicalSurgery/" + queueId);
                    $("#btnQuotationPatientIpd").click(function(){
                        w=window.open();
                        w.document.write('<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">');
                        w.document.write('<link rel="stylesheet" type="text/css" href="<?php echo $this->webroot; ?>css/style.css" /><link rel="stylesheet" type="text/css" href="<?php echo $this->webroot; ?>css/table.css" /><link rel="stylesheet" type="text/css" href="<?php echo $this->webroot; ?>css/button.css" /><link rel="stylesheet" type="text/css" href="<?php echo $this->webroot; ?>css/print.css" media="print" />');                        
                        ...
                        w.document.write($("#quotationPatientIpd").html());
                        w.document.close();
                        try
                        {
                            //Run some code here                                                                                                       
        ...
        $(".btnBackPatientMedicalSurgery").dblclick(function(event){
            ...
            $('#PatientIpdEditMedicalSurgeryForm').validationEngine('hideAll');
    ...
    function checkBfSavePatient(){
        var formName = "#PatientIpdEditMedicalSurgeryForm";
...
<fieldset id="patientIdpManagementInfo">
    ...
    <table style="width: 100%;" cellspacing="0">
        ...
        <tr>
            ...
            <td>
                <?php echo $this->Form->text('authorized_expiration_date', array('value' => $this->data['PatientIpd']['authorized_expiration_date'] != "0000-00-00" ? $this->data['PatientIpd']['authorized_expiration_date'] : "")); ?>
            </td>
        </tr>
        <tr>
