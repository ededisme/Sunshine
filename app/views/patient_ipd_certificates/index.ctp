...
<script type="text/javascript">
    var oTablePatientCertificate;
    var tabSalesId  = $(".ui-tabs-selected a").attr("href");
    var tabSalesReg = '';
    $(document).ready(function(){        
        oTablePatientCertificate = $("#<?php echo $tblName; ?>").dataTable({
            "bProcessing": true,
            "bServerSide": true,
            ...
            "fnInfoCallback": function( oSettings, iStart, iEnd, iMax, iTotal, sPre ) {
                ...
                $(".btnViewPatientCertificate").click(function(event){
                ...
                $(".btnEditPatientCertificate").click(function(event){
                    event.preventDefault();
                    var id = $(this).attr('rel');
                    var leftPanel  = $(this).parent().parent().parent().parent().parent().parent().parent();
                    var rightPanel = leftPanel.parent().find(".rightPanel");
                ...
                $(".btnDeletePatientCertificate").click(function(event){
                    event.preventDefault();
                    var id = $(this).attr('rel');
			...
			buttons: {
                ...
                $(".btnPrintPatientCertificate").click(function(event){
                    event.preventDefault();
                    var id = $(this).attr('rel');
                    $("#dialog").html('<div class="buttons"><button type="submit" class="positive reprintPatientCertificate" ><img src="<?php echo $this->webroot; ?>img/button/printer.png" alt=""/><span class="txtReprintPatientIPDCertificate"><?php echo ACTION_PRINT_PATIENT_IPD_CERTIFICATE; ?></span></button></div> ');
                    $(".reprintPatientCertificate").click(function(){
                        $.ajax({
                            type: "POST",
                            url: "<?php echo $this->base . '/' . $this->params['controller']; ?>/printPatientIpdCertificate/"+id,
                            beforeSend: function(){
                                $(".loader").attr('src','<?php echo $this->webroot; ?>img/layout/spinner.gif');
                            },
        ...
        $(".btnAddPatientCertificate").click(function(event){
            event.preventDefault();
...
<div class="leftPanel">
    <div style="padding: 5px;border: 1px dashed #bbbbbb;">
        <?php if($allowAdd){ ?>
        <div class="buttons">
            <a href="" class="positive btnAddPatientCertificate">
                <img src="<?php echo $this->webroot; ?>img/button/plus.png" alt=""/>
                <?php echo MENU_PATIENT_IPD_CERTIFICATE_ADD; ?>
            </a>
        </div>
        <?php } ?>        
