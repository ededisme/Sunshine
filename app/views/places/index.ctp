...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        oTablePlace = $("#<?php echo $tblName; ?>").dataTable({
            "bProcessing": true,
            "bServerSide": true,
            ...
            "fnInfoCallback": function( oSettings, iStart, iEnd, iMax, iTotal, sPre ) {
                ...
                $(".btnViewPlace").click(function(event){
                ...
                $(".btnEditPlace").click(function(event){
                    event.preventDefault();
                    var id = $(this).attr('rel');
                    var leftPanel=$(this).parent().parent().parent().parent().parent().parent().parent();
                    var rightPanel=leftPanel.parent().find(".rightPanel");
                ...
                $(".btnDeletePlace").click(function(event){
                    event.preventDefault();
                    var id = $(this).attr('rel');
                            ...
                            '<?php echo ACTION_DELETE; ?>': function() {
                                $.ajax({
                                    ...
                                    success: function(result){
                                        ...
                                        oTablePlace.fnDraw(false);
                                        // alert message
                                        if(result != '<?php echo MESSAGE_DATA_HAS_BEEN_DELETED; ?>'){
                                            createSysAct('Place', 'Delete', 2, result);
                                            $("#dialog").html('<p><span class="ui-icon ui-icon-info" style="float:left; margin:0 7px 20px 0;"></span><?php echo MESSAGE_PROBLEM; ?></p>');
                                        }else {
                                            createSysAct('Place', 'Delete', 1, '');
                                            // alert message
        ...
        $(".btnExportPlace").click(function(){
            $.ajax({
                type: "POST",
                url: "<?php echo $this->base; ?>/<?php echo $this->params['controller']; ?>/exportExcel",
                data: "action=export",
                beforeSend: function(){
                    $(".btnExportPlace").attr('disabled','disabled');
                    $(".btnExportPlace").find('img').attr("src", "<?php echo $this->webroot; ?>img/layout/spinner.gif");
                },
                success: function(){
                    $(".btnExportPlace").removeAttr('disabled');
                    $(".btnExportPlace").find('img').attr("src", "<?php echo $this->webroot; ?>img/button/csv.png");
                    window.open("<?php echo $this->webroot; ?>public/report/positions_export.csv", "_blank");
                }
            });
        });
        $(".btnAddPlace").click(function(event){
            event.preventDefault();
...
<div class="leftPanel">
    <div style="padding: 5px;border: 1px dashed #bbbbbb;">
        <?php if($allowAdd){ ?>
        <div class="buttons">
            <a href="" class="positive btnAddPlace">
                <img src="<?php echo $this->webroot; ?>img/button/plus.png" alt=""/>
                <?php echo MENU_PLACE_ADD; ?>
            </a>
        </div>
        <?php } ?>
        <?php if($allowExport){ ?>
        <div class="buttons">
            <button type="button" class="positive btnExportPlace">
