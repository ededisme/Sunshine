...
<script type="text/javascript">
    $(document).ready(function() {
        // Prevent Key Enter
        preventKeyEnter();
        $("#LaboTitleGroupEditForm").validationEngine();
        $("#LaboTitleGroupEditForm").ajaxForm({
            ...
            success: function(result) {
                ...
                var rightPanel=$("#LaboTitleGroupEditForm").parent();
...
<?php $title = $titles['LaboTitleGroup']['name'] ?>
<fieldset>
        ...
        <tr>
            <td style="vertical-align: top;">
                <select id="s" multiple="multiple" style="width: 300px; height: 200px;">
                    <?php
                    $querySource = mysql_query("SELECT DISTINCT labo_item_groups.id,CONCAT(labo_item_groups.name,' - ',companies.name) AS name FROM labo_item_groups LEFT JOIN companies ON labo_item_groups.company_id = companies.id  WHERE labo_item_groups.is_active=1 AND companies.is_active=1 AND labo_item_groups.id NOT IN (" . $this->data['LaboTitleGroup']['labo_item_group_id'] . ") ORDER BY labo_item_groups.name ASC");
            ...
            <td style="vertical-align: top;">                
                <select id="d" name="data[LaboTitleGroup][labo_item_group_id][]" multiple="multiple" style="width: 300px; height: 200px;">
                    <?php
                    $querySource = mysql_query("SELECT DISTINCT labo_item_groups.id,CONCAT(labo_item_groups.name,' - ',companies.name) AS name FROM labo_item_groups LEFT JOIN companies ON labo_item_groups.company_id = companies.id  WHERE labo_item_groups.is_active=1 AND companies.is_active=1 AND labo_item_groups.id IN (" . $this->data['LaboTitleGroup']['labo_item_group_id'] . ") ORDER BY labo_item_groups.name ASC");
                    while ($dataSource = mysql_fetch_array($querySource)) {
                    ?>
                        <option value="<?php echo $dataSource['id']; ?>"><?php echo $dataSource['name']; ?></option>
                    <?php } ?>
