 ...
 */
$aColumns = array("Inventory.product_id", "Inventory.lots_number", "Inventory.expired_date", "(SELECT name FROM locations WHERE id = Inventory.location_id)", "Inventory.total_qty", "Inventory.location_id");
 ...
 * Get data to display
 */
$group_by = "GROUP BY Inventory.location_id, Inventory.lots_number, Inventory.expired_date";
$sQuery = "
...
while ($aRow = mysql_fetch_array($rResult)) {
    $row = array();
    $totalOrder = 0;
    $sqlOrder = mysql_query("SELECT SUM(qty) FROM stock_orders WHERE purchase_return_id IS NULL AND product_id = ".$productId." AND location_group_id = ".$locationGroupId." AND location_id = ".$aRow[5]." AND lots_number = '".$aRow[1]."' AND expired_date = '".$aRow[2]."'");
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        if ($i == 0) {
            ...
            $row[] = '<input type="checkbox" uom="'.$smallUomId.'" location-id="'.$aRow[5].'" value="'.$aRow[0].'" expired="'.$aRow[2].'" lots="'.$aRow[1].'" name="chkPickProductBillReturn[]" />';
        ...
        } else if ($aColumns[$i] == 'Inventory.total_qty') {
