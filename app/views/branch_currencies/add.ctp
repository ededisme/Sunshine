...
<script type="text/javascript">
    $(document).ready(function(){
        // Prevent Key Enter
        preventKeyEnter();
        $("#BranchCurrencyAddForm").validationEngine('attach', {
            isOverflown: true,
            overflownDIV: ".ui-tabs-panel"
        });
        $("#BranchCurrencyAddForm").ajaxForm({
            ...
            success: function(result) {
                ...
                if(result != '<?php echo MESSAGE_DATA_HAS_BEEN_SAVED; ?>' && result != '<?php echo MESSAGE_DATA_COULD_NOT_BE_SAVED; ?>' && result != '<?php echo MESSAGE_DATA_ALREADY_EXISTS_IN_THE_SYSTEM; ?>'){
                    createSysAct('Branch Currency', 'Add', 2, result);
                    $("#dialog").html('<p><span class="ui-icon ui-icon-info" style="float:left; margin:0 7px 20px 0;"></span><?php echo MESSAGE_PROBLEM; ?></p>');
                }else {
                    createSysAct('Branch Currency', 'Add', 1, '');
                    // alert message
...
<br />
<?php echo $this->Form->create('BranchCurrency', array('inputDefaults' => array('div' => false, 'label' => false))); ?>
<fieldset>
    ...
    <table>
            ...
            <td>
                <div class="inputContainer">
                    <select name="data[BranchCurrency][branch_id]" id="BranchCurrencyBranchId" class="validate[required]">
                        ...
                        ?>
                        <option value="<?php echo $branch['Branch']['id']; ?>" dcur="<?php echo $branch['Branch']['currency_center_id']; ?>"><?php echo $branch['Branch']['name']; ?></option>
                        <?php } ?>
                    </select>
                </div>
            </td>
        ...
        <tr>
            ...
            <td>
                <div class="inputContainer">
                    <?php echo $this->Form->input('currency_center_id', array('class' => 'validate[required]', 'empty' => INPUT_SELECT, 'disabled' => true)); ?>
                </div>
            </td>
        </tr>
    </table>
