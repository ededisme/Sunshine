...
<script type="text/javascript">
    $(document).ready(function() {
        // Prevent Key Enter
        preventKeyEnter();
        $("#AgeForLaboEditForm").validationEngine();
        $("#AgeForLaboEditForm").ajaxForm({
...
<fieldset>
    <?php                      
            ...
            $year_to = 0;
            $month_to = 0;
            if($this->data['AgeForLabo']['from']>12){
                $year_from_first = $this->data['AgeForLabo']['from']/12;
                $year_from = number_format($year_from_first);
                $month_from = ($year_from_first-$year_from)*12;
            }else{
                $month_from = $this->data['AgeForLabo']['from'];
            }                    
            if($this->data['AgeForLabo']['to']>12){
                $year_to_first = $this->data['AgeForLabo']['to']/12;
                $year_to = number_format($year_to_first);
                $month_to = ($year_to_first-$year_to)*12;                          
            }else{
                $month_to = $this->data['AgeForLabo']['to'];
            }   
    ?>
    ...
    <table style="width: 100%;" cellspacing="0">
        ...
        <tr>
            ...
            <td><?php echo $this->Form->text('from_year', array('value' => $year_from, 'class' => 'validate[custom[number]]', 'onkeypress'=>'return isNumberKey(event)')); ?>Year</td>                            
            <td><?php echo $this->Form->text('from_month', array('value' => $month_from, 'class' => 'validate[custom[number]]', 'onkeypress'=>'return isNumberKey(event)')); ?>Month</td>
        </tr>
        <tr>
            <td><label for="AgeForLaboTo"><?php echo REPORT_TO; ?> :</label></td>
            <td><?php echo $this->Form->text('to_year', array('value' => $year_to, 'class' => 'validate[custom[number]]', 'onkeypress'=>'return isNumberKey(event)')); ?>Year</td>                            
            <td><?php echo $this->Form->text('to_month', array('value' => $month_to, 'class' => 'validate[custom[number]]', 'onkeypress'=>'return isNumberKey(event)')); ?>Month</td>
        </tr>
    </table>
</fieldset>
<br />
