 ...
 */
$aColumns = array('CONCAT(cus.id,"||",IFNULL(cus.payment_term_id, 0),"||",IFNULL(location_groups.id, 0))', 'cus.customer_code', 'cus.name_kh', 'cus.name', 'cus.sex', 'cus.main_number', 'photo');
...
while ($aRow = mysql_fetch_array($rResult)) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        if ($i == 0) {
            ...
            $index ++;
            $row[] = '<input type="radio" limit-balance="0" limit-invoice="0" inv-used="0" bal-used="0" value="'.$value[0].'" loc-group="'.$value[2].'" name="chkCustomer" photo="'.$aRow[6].'" name-kh="'.$aRow[2].'" name-us="'.$aRow[3].'" rel="'.$aRow[1].'" term-id="'.$value[1].'" net_days="'.$aRow[6].'" ptype="'.$pType.'" />' ;
        } else if ($i == 6) {
