...
$aColumns = array("products.id", 
                  ...
                  'products.code' , 
                  'products.name',
                  "CONCAT_WS('--',u.name,u.id,products.small_val_uom)");
...
while ($aRow = mysql_fetch_array($rResult)) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        if ($i == 0) {
            // Assign Product Record As JSON
            $row[] = "<input type='radio' value='".$aRow[2]."' name='chkProductTO' />";
        } else if ($i == 4) {
            $row[] = $uom[0];
        } else if ($aColumns[$i] != ' ') {
            /* General output */
