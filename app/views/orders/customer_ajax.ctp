 ...
 */
$aColumns = array('CONCAT(p.id,"||",0,"||",0,"||",0,"||",q.id)', 'p.patient_code', 'p.patient_name', 'p.telephone');
//$aColumns = array('cus.id', 'cus.customer_code', 'CONCAT(cus.name_kh," (",cus.name,")")', 'cus.main_number', 'cus.main_number');
...
//    }
    for ($i = 0; $i < count($aColumns); $i++) {
        if ($i == 0) {
            ...
            $index ++;
            //$row[] = '<input type="radio" value="'.$aRow[0].'" name="chkCMCustomer" code="'.$aRow[1].'" rel="'.$aRow[2].'" ptype="'.$pType.'" />' ;
            $row[] = '<input type="radio" value="'.$aRow[0].'" name="chkCMCustomer" code="'.$aRow[1].'" rel="'.$aRow[2].'"/>' ;
        } else if ($i == 4) {
