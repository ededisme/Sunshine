...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        if($allowAddSection){
        ?>
        $("#ServiceSectionId").chosen({ width: 410, allow_add: true, allow_add_label: '<?php echo MENU_ADD_SECTION; ?>', allow_add_id: 'addNewSectionService' });
        $("#addNewSectionService").click(function(){
            $.ajax({
                ...
                success: function(msg){
                    ...
                    $("#dialog").html(msg);
                    $("#dialog").dialog({
                        title: '<?php echo MENU_ADD_SECTION; ?>',
                        resizable: false,
                        modal: true,
                        width: '450',
                        height: '300',
        ...
        $("#ServiceAddForm").validationEngine('attach', {
            isOverflown: true,
            overflownDIV: ".ui-tabs-panel"
        });
        $("#ServiceAddForm").ajaxForm({
    ...
    function getBranchProduct(){
        ...
        if(companyId != ''){
            $.ajax({
                ...
                beforeSend: function(){
                    listbox_selectall('serviceBranchSelected', true);
                    listbox_moveacross('serviceBranchSelected', 'serviceBranch');
                    $("#serviceBranch").html('');
                    $(".loader").attr("src","<?php echo $this->webroot; ?>img/layout/spinner.gif");
                },
                success: function(opt){
                    $(".loader").attr("src","<?php echo $this->webroot; ?>img/layout/spinner-placeholder.gif");
                    $("#serviceBranch").append(opt);
                    // Auto Select Branch
                    listbox_selectall('serviceBranch', true);
                    listbox_moveacross('serviceBranch', 'serviceBranchSelected');
                }
            });
        }
    }
...
<fieldset>
    ...
    <table>
        <tr>
            <td><label for="ServiceCode"><?php echo TABLE_CODE; ?> :</label></td>
            <td>
        ...
        <tr>
            ...
            <td>
                <div class="inputContainer">
                    <?php echo $this->Form->text('unit_price', array('class' => 'float')); ?> ($)
                </div>
            </td>
        </tr>
        -->
            ...
            <td>
                <div class="inputContainer">
                    <select id="ServiceChartAccountId" name="data[Service][chart_account_id]" class="validate[required]">
                        ...
                        ?>
                        <option value="<?php echo $data[0]['id']; ?>" chart_account_type_name="<?php echo $data[0]['chart_account_type_name']; ?>" company_id="<?php echo $data[0]['company_id']; ?>" <?php echo $data[0]['id']==$serviceAccountId?'selected="selected"':''; ?>><?php echo $data[0]['name']; ?></option>
                        <?php } ?>
                    </select>
                </div>
            </td>
    ...
    <table>
        ...
        <tr>
            <td style="vertical-align: top;">
                <select id="serviceBranch" multiple="multiple" style="width: 280px; height: 200px;"></select>
            </td>
            ...
            <td style="vertical-align: top;">
                <select id="serviceBranchSelected" name="data[Service][branch_id][]" multiple="multiple" style="width: 280px; height: 200px;"></select>
            </td>
        </tr>
    </table>
</fieldset>
...
<fieldset>
    ...
    <table id="example" class="table" cellspacing="0">
        ...
        <tr class="ServiceTr">
            ...
            <td>
                <?php echo $this->Form->input('patient_group_id', array('name' => 'data[Service][patient_group_id][]', 'empty' => SELECT_OPTION, 'label' => false, 'class' => 'servicePatientGroup validate[required]', 'style' => 'width:220px;')); ?>
            </td>
            <td>
                <?php echo $this->Form->text('unit_price', array('name' => 'data[Service][price][]', 'class' => 'unit_price float validate[required]', 'style' => 'width:200px;')); ?> 
            </td>
            <td>
