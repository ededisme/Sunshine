...
$allowAdd=checkAccess($user['User']['id'], 'customers', 'add');
    ...
    $(document).ready(function(){
                                ...
                                $.ajax({
                                    ...
                                    success: function(result){
                                        ...
                                        if(result != '<?php echo MESSAGE_DATA_HAS_BEEN_DELETED; ?>'){
                                            createSysAct('Customer', 'Delete', 2, result);
                                            $("#dialog").html('<p><span class="ui-icon ui-icon-info" style="float:left; margin:0 7px 20px 0;"></span><?php echo MESSAGE_PROBLEM; ?></p>');
                                        }else {
                                            createSysAct('Customer', 'Delete', 1, '');
                                            // alert message
        ...
        $(".btnExportCustomer").click(function(){
            $.ajax({
                ...
                data: "action=export",
                beforeSend: function(){
                    $(".btnExportCustomer").attr('disabled','disabled');
                    $(".btnExportCustomer").find('img').attr("src", "<?php echo $this->webroot; ?>img/layout/spinner.gif");
                },
                success: function(){
                    $(".btnExportCustomer").removeAttr('disabled');
                    $(".btnExportCustomer").find('img').attr("src", "<?php echo $this->webroot; ?>img/button/csv.png");
                    window.open("<?php echo $this->webroot; ?>public/report/customer_export.csv", "_blank");
                }
            });
        });
...
<div class="leftPanel">
    <div style="padding: 5px;border: 1px dashed #bbbbbb;">
        <?php if($allowAdd){ ?>
        <div class="buttons">
            <a href="" class="positive btnAddCustomer">
                <img src="<?php echo $this->webroot; ?>img/button/plus.png" alt=""/>
                <?php echo MENU_CUSTOMER_MANAGEMENT_ADD; ?>
            </a>
        </div>
        <?php } ?>
        <?php if($allowExport){ ?>
        <div class="buttons">
            <button type="button" class="positive btnExportCustomer">
