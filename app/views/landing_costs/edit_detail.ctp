...
<div id="bodyList">
    <table id="tblLandingCost" class="table" cellspacing="0" style="padding: 0px; width:100%">
        ...
        foreach($landingCostDetails AS $landingCostDetail){
            $rnd = rand();
            $productName = str_replace('"', '&quot;', $landingCostDetail['Product']['name']);
            $uomName = '';
            if(!empty($landingCostDetail['LandingCostDetail']['qty_uom_id'])){
                $sqlUom = mysql_query("SELECT abbr FROM uoms WHERE id=".$landingCostDetail['LandingCostDetail']['qty_uom_id'].";");
        ...
        ?>
        <tr class="tblLandingCostList">
            <td class="first" style="width:5%; text-align: center;padding: 0px; height: 30px;"><?php echo ++$index; ?></td>
            <td style="width:10%; text-align: left; padding: 5px;"><input type="text" readonly="" class="lblUPC" value="<?php echo $landingCostDetail['Product']['barcode']; ?>" /></td>
            <td style="width:10%; text-align: left; padding: 5px;"><input type="text" readonly="" class="lblSKU" value="<?php echo $landingCostDetail['Product']['code']; ?>" /></td>
            <td style="width:23%; text-align: left; padding: 5px;">
                <div class="inputContainer" style="width:100%">
                    <input type="hidden" name="purchase_order_detail[]" value="<?php echo $landingCostDetail['LandingCostDetail']['purchase_order_detail_id']; ?>" />
                    <input type="hidden" name="conversion[]" value="<?php echo $landingCostDetail['LandingCostDetail']['conversion']; ?>" />
                    <input type="hidden" name="small_val_uom[]" value="<?php echo $landingCostDetail['Product']['small_val_uom']; ?>" />
                    <input type="hidden" class="product_id" value="<?php echo $landingCostDetail['LandingCostDetail']['product_id']; ?>" name="product_id[]" />
                    <input type="text" value="<?php echo$productName; ?>" name="product[]" class="product validate[required]" style="width: 90%;" />
                </div>
            </td>
            <td style="width:8%; text-align: center;padding: 0px;">
                <div class="inputContainer" style="width:100%">
                    <input type="hidden" name="qty[]" value="<?php echo $landingCostDetail['LandingCostDetail']['qty']; ?>" />
                    <?php echo number_format($landingCostDetail['LandingCostDetail']['qty'], 0); ?>
                </div>
            </td>
            <td style="width:15%; padding: 0px; text-align: center">
                <div class="inputContainer" style="width:100%">
                    <input type="hidden" name="qty_uom_id[]" value="<?php echo $landingCostDetail['LandingCostDetail']['qty_uom_id']; ?>" />
                    <?php echo $uomName; ?>
                </div>
            </td>
            <td style="width:11%; padding: 0px; text-align: center">
                <div class="inputContainer" style="width:100%">
                    <input type="hidden" name="unit_cost[]" value="<?php echo $landingCostDetail['LandingCostDetail']['unit_cost']; ?>" />
                    <?php echo number_format($landingCostDetail['LandingCostDetail']['unit_cost'], $rowOption[0]); ?>
                </div>
            </td>
            <td style="width:11%; text-align: center; padding: 0px;">
                <div class="inputContainer" style="width:100%">
                    <input type="text" value="<?php echo number_format($landingCostDetail['LandingCostDetail']['landed_cost'], $rowOption[0]); ?>" id="landedCost_<?php echo $rnd; ?>" name="landed_cost[]" style="width:84%" class="float landedCost" />
                </div>
            </td>
            <td style="width:7%">
                <img alt="Remove" src="<?php echo $this->webroot; ?>img/button/cross.png" class="btnRemoveLandingCost" align="absmiddle" style="cursor: pointer;" onmouseover="Tip('Remove')" />
                &nbsp; <img alt="Up" src="<?php echo $this->webroot; ?>img/button/move_up.png" class="btnMoveUpLandingCost" align="absmiddle" style="cursor: pointer;" onmouseover="Tip('Up')" />
                &nbsp; <img alt="Down" src="<?php echo $this->webroot; ?>img/button/move_down.png" class="btnMoveDownLandingCost" align="absmiddle" style="cursor: pointer;" onmouseover="Tip('Down')" />
            </td>
        </tr>
        <?php
        }
