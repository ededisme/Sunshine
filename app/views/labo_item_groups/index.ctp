...
<script type="text/javascript">
    var oTableLaboItemGroup;
    $(document).ready(function(){
        // Prevent Key Enter
        preventKeyEnter();
        $("#<?php echo $tblName; ?> td:first-child").addClass('first');
        oTableLaboItemGroup = $("#<?php echo $tblName; ?>").dataTable({
            "bProcessing": true,
            "bServerSide": true,
            ...
            "fnInfoCallback": function( oSettings, iStart, iEnd, iMax, iTotal, sPre ) {
                ...
                $(".btnViewLaboItemGroup").click(function(event){
                ...
                $(".btnEditLaboItemGroup").click(function(event){
                    event.preventDefault();
                    var id = $(this).attr('rel');
                    var name = $(this).attr('name');
                    var leftPanel=$(this).parent().parent().parent().parent().parent().parent().parent();
                ...
                $(".btnDeleteLaboItemGroup").click(function(event){
                    event.preventDefault();
                    var id = $(this).attr('rel');
                    var name = $(this).attr('name');
        ...
        $(".btnAddLaboItemGroup").click(function(event){
            event.preventDefault();
        ...
        $(".btnExportSubGroup").click(function(){
            $.ajax({
                type: "POST",
                url: "<?php echo $this->base; ?>/<?php echo $this->params['controller']; ?>/exportExcel",
                data: "action=export",
                beforeSend: function(){
                    $(".btnExportSubGroup").attr('disabled','disabled');
                    $(".btnExportSubGroup").find('img').attr("src", "<?php echo $this->webroot; ?>img/layout/spinner.gif");
                },
                success: function(){
                    $(".btnExportSubGroup").removeAttr('disabled');
                    $(".btnExportSubGroup").find('img').attr("src", "<?php echo $this->webroot; ?>img/button/csv.png");
                    window.open("<?php echo $this->webroot; ?>public/report/labo_sub_group_export.csv", "_blank");
                }
            });
        });
    });
...
<div class="leftPanel">
    <div style="padding: 5px;border: 1px dashed #3C69AD;">
        <?php if($allowAdd){ ?>
        <div class="buttons">
            <a href="" class="positive btnAddLaboItemGroup">
                <img src="<?php echo $this->webroot; ?>img/button/plus.png" alt=""/>
                <?php echo MENU_SUB_GROUP_ADD; ?>
        ...
        <?php if($allowExport){ ?>
        <div class="buttons">
            <button type="button" class="positive btnExportSubGroup">
