<?php
    ...
    $small_label = "";
    $sqlUomSm    = mysql_query("SELECT abbr FROM uoms WHERE id = (SELECT to_uom_id FROM uom_conversions WHERE from_uom_id = {$consignmentDetail['Product']['price_uom_id']} AND is_small_uom = 1 LIMIT 1)");
    if(mysql_num_rows($sqlUomSm)){
        $rowUomSm = mysql_fetch_array($sqlUomSm);
        $small_label = $rowUomSm['abbr'];
    }
    $sqlUomMain = mysql_query("SELECT abbr FROM uoms WHERE id = {$consignmentDetail['Product']['price_uom_id']};");
    $rowUomMain = mysql_fetch_array($sqlUomMain);
    $main_uom   = $rowUomMain['abbr'];
    if($small_label == ''){
        $small_label = $main_uom;
    }
    $totalDn = ($consignmentDetail['ConsignmentDetail']['qty'] * $consignmentDetail['ConsignmentDetail']['conversion']);
    $tblName = "tbl" . rand(); 
?>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $("#<?php echo $tblName; ?>").dataTable({
            ...
            "bProcessing": true,
            "bServerSide": true,
            "sAjaxSource": "<?php echo $this->base . '/' . $this->params['controller']; ?>/pickProductAjax/<?php echo $consignmentDetail['ConsignmentDetail']['product_id']; ?>/<?php echo $locationGroupId; ?>/<?php echo $small_label; ?>",
            ...
            "fnInfoCallback": function( oSettings, iStart, iEnd, iMax, iTotal, sPre ) {
                ...
                $("input[name='chkPickProductConsignment[]']").click(function(){
                    var thisCheck = $(this);
...
<div style="padding: 5px;border: 1px dashed #bbbbbb; margin-bottom: 5px;">
    <div style="float:left;">
        <b><?php echo TABLE_PRODUCT; ?>:</b> <?php echo $consignmentDetail['Product']['code']." ".$consignmentDetail['Product']['name']; ?> 
        <br/><b>Total Order: <span id="total_order"><?php echo $totalDn; ?></span> <?php echo $small_label; ?></b> 
    </div>
    <div style="clear: both;"></div>
</div>
<center style="color: #C00; font-size: 14px; display:none" id="warningPickProductConsignment">Please check box and fill in quality for pick product!</center>
<br />
<div id="dynamic">
