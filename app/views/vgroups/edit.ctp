...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        // Prevent Key Enter
        preventKeyEnter();
        $("#VgroupEditForm").validationEngine();
        $("#VgroupEditForm").ajaxForm({
            ...
            success: function(result) {
                ...
                if(result != '<?php echo MESSAGE_DATA_HAS_BEEN_SAVED; ?>' && result != '<?php echo MESSAGE_DATA_COULD_NOT_BE_SAVED; ?>'){
                    createSysAct('Vgroup', 'Edit', 2, result);
                    $("#dialog").html('<p><span class="ui-icon ui-icon-info" style="float:left; margin:0 7px 20px 0;"></span><?php echo MESSAGE_PROBLEM; ?></p>');
                }else {
                    createSysAct('Vgroup', 'Edit', 1, '');
                    // alert message
        ...
        $("#VgroupEmployee").autocomplete("<?php echo $this->base . "/vendors/searchVendor"; ?>", {
            width: 410,
            max: 10,
            highlight: false,
            scroll: true,
...
<fieldset>
    ...
    <table>
        <tr>
            ...
            <td>
                <?php echo $this->Form->text('vendor', array('id' => 'VgroupVendor')); ?>
        ...
        <tr>
            ...
            <td style="vertical-align: top;">
                <select id="vendor_id" name="data[Vgroup][vendor_id][]" multiple="multiple" style="width: 420px; height: 150px;">
                    <?php
                    $query = "SELECT DISTINCT vendors.id, vendors.vendor_code, CONCAT(vendors.name) AS fullname FROM vendors
                                INNER JOIN vendor_vgroups ON vendors.id = vendor_vgroups.vendor_id
                                WHERE vendors.is_active=1 AND vendor_vgroups.vgroup_id=" . $this->data['Vgroup']['id'];
                    $querySource = mysql_query($query);
                    while ($dataSource = mysql_fetch_array($querySource)) {
                    ?>
                        <option value="<?php echo $dataSource['id']; ?>" rel="<?php echo $dataSource['fullname']; ?>" ><?php echo $dataSource['vendor_code'] . " - " . $dataSource['fullname']; ?></option>
                    <?php } ?>
                </select>
            </td>
        </tr>
