...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        checkEventCM();
        <?php  
        if(!empty($credit_memo)) {
            $priceTypeId = $credit_memo['CreditMemo']['price_type_id'];
            $sqlPriceType = mysql_query("SELECT GROUP_CONCAT(price_type_id) FROM cgroup_price_types WHERE cgroup_id IN (SELECT cgroup_id FROM customer_cgroups WHERE customer_id = ".$credit_memo['CreditMemo']['customer_id']." GROUP BY cgroup_id)");
        ...
        customerPriceTypeCM('<?php echo $rowPriceType[0]; ?>', <?php echo $priceTypeId; ?>);
        <?php
        } else {
        ?>
        changeInputCSSCM();
    ...
    function checkItemLockSalesReturn(){
        var isLock = false;
        $(".tblCMList").find(".product_id").each(function(){
        ...
        if(isLock == true){
            $("#CreditMemoOrderDate").attr("disabled", true);
            $("#CreditMemoOrderDate").removeAttr("name");
            $("#tmpCreditMemoOrderDate").attr("name", "data[PurchaseOrder][order_date]");
        } else {
            $('#CreditMemoOrderDate').datepicker({
                dateFormat:'dd/mm/yy',
                changeMonth: true,
                changeYear: true
            }).unbind("blur");

            $("#CreditMemoOrderDate").datepicker("option", "minDate", "<?php echo $dataClosingDate[0]; ?>");
            $("#CreditMemoOrderDate").datepicker("option", "maxDate", 0);
        }
    }
</script>
    ...
    <table id="tblCM" class="table" cellspacing="0" style="padding: 0px; width:100%">
        <tr id="OrderListAddCM" class="tblCMList" style="visibility: hidden;" >
            ...
            <td style="width:9%; text-align: center; padding: 0px;">
                ...
                <input type="text" <?php if(!$allowEditPrice){ ?>readonly="readonly"<?php } ?> name="total_price[]" value="0" style="width:84%; height: 25px;" class="float total_price" />
            </td>
            <td style="width:4%">
        ...
        if(!empty($creditMemoDetails)){
            foreach($creditMemoDetails AS $creditMemoDetail){
                ...
                $totalQtyByDate = 0;
                $sqlInv = mysql_query("SELECT SUM(qty) FROM inventories WHERE location_group_id = ".$credit_memo['CreditMemo']['location_group_id']." AND location_id = ".$credit_memo['CreditMemo']['location_id']." AND product_id = ".$creditMemoDetail['Product']['id']." AND date <= '".$credit_memo['CreditMemo']['order_date']."' AND date_expired = '".$creditMemoDetail['CreditMemoDetail']['expired_date']."'");
                ...
                // Total Qty On Hand
                $totalQtyOnHand = 0;
                $sqlOnHand = mysql_query("SELECT SUM(total_qty - total_order) FROM ".$credit_memo['CreditMemo']['location_id']."_inventory_totals WHERE product_id = ".$creditMemoDetail['Product']['id']." AND expired_date = '".$creditMemoDetail['CreditMemoDetail']['expired_date']."'");
                ...
                $sqlProCus   = mysql_query("SELECT name FROM product_with_customers WHERE product_id = ".$creditMemoDetail['Product']['id']." AND customer_id = ".$credit_memo['CreditMemo']['customer_id']." ORDER BY created DESC LIMIT 1");
        ...
        <tr class="tblCMList">
            ...
            <td style="width:10%; text-align: left; padding: 5px;">
                <input type="text" readonly="" style="width: 95%; height: 25px;" class="lblUPC" value="<?php echo $creditMemoDetail['Product']['barcode']; ?>" />
            </td>
            <td style="width:18%; text-align: left; padding: 5px;">
                <div class="inputContainer" style="width:100%;">
                    <input type="hidden" class="itemIsLock" value="<?php echo $isLock; ?>" />
                    <input type="hidden" name="product_id[]" value="<?php echo $creditMemoDetail['CreditMemoDetail']['product_id']; ?>" class="product_id" />
                    <input type="hidden" name="service_id[]" />
                    <input type="hidden" name="discount_id[]" value="<?php echo $creditMemoDetail['CreditMemoDetail']['discount_id']; ?>" />
                    <input type="hidden" name="discount_amount[]" value="<?php echo $creditMemoDetail['CreditMemoDetail']['discount_amount']; ?>" />
                    <input type="hidden" name="discount_percent[]" value="<?php echo $creditMemoDetail['CreditMemoDetail']['discount_percent']; ?>" />
                    <input type="hidden" name="cm_conversion[]" class="cm_conversion" value="<?php echo $creditMemoDetail['CreditMemoDetail']['conversion']; ?>" />
                    <input type="hidden" name="small_val_uom[]" class="small_val_uom" value="<?php echo $creditMemoDetail['Product']['small_val_uom']; ?>" />
                    <input type="hidden" name="note[]" id="note" readonly="readonly" value="<?php echo $creditMemoDetail['CreditMemoDetail']['note']; ?>" class="note" />
                    <input type="hidden" class="orgProName" value="<?php echo "PUC: ".htmlspecialchars($creditMemoDetail['Product']['barcode'], ENT_QUOTES, 'UTF-8')."<br/><br/>SKU: ".htmlspecialchars($creditMemoDetail['Product']['code'], ENT_QUOTES, 'UTF-8')."<br/><br/>Name: ".htmlspecialchars($creditMemoDetail['Product']['name'], ENT_QUOTES, 'UTF-8'); ?>" />
                    <input type="text" id="product_<?php echo $index; ?>" name="product[]" class="product validate[required]" value="<?php echo str_replace('"', '&quot;', $productName); ?>" style="width: 75%; height: 25px;" />
                    <img alt="Note" src="<?php echo $this->webroot . 'img/button/note.png'; ?>" class="noteAddCM" align="absmiddle" style="cursor: pointer;" onmouseover="Tip('Note')" />
                    <img alt="Information" src="<?php echo $this->webroot . 'img/button/view.png'; ?>" class="btnProductCMInfo" align="absmiddle" style="cursor: pointer;" onmouseover="Tip('Information')" />
                </div>
            </td>
            <td style="width:7%; text-align: center;padding: 0px; <?php if($rowSettingUomDetail[0] == 0){ ?>display: none;<?php } ?>">
                <div class="inputContainer" style="width:100%">
                    <input type="text" id="lots_number_<?php echo $index; ?>" value="<?php echo $creditMemoDetail['CreditMemoDetail']['lots_number']; ?>" name="lots_number[]" style="width:90%; height: 25px;" class="lots_number <?php if($rowSettingUomDetail[0] == 1){ ?>validate[required]<?php } ?>" />
                </div>
            </td>
            <td style="width:10%; text-align: center;padding: 0px;">
                <div class="inputContainer" style="width:100%">
                    ...
                    if($isLock == 1){
                    ?>
                    <input type="hidden" name="expired_date[]" value="<?php echo $creditMemoDetail['CreditMemoDetail']['expired_date']; ?>" />
                    <input type="text" value="<?php if($creditMemoDetail['CreditMemoDetail']['expired_date']!='' && $creditMemoDetail['CreditMemoDetail']['expired_date']!='0000-00-00'){ echo dateShort($creditMemoDetail['CreditMemoDetail']['expired_date']); } else { echo '0000-00-00'; } ?>" style="width:90%; height: 25px; <?php if($creditMemoDetail['Product']['is_expired_date'] == 0){ ?>visibility: hidden;<?php } ?>" class="expired_date" disabled="" />
                    <?php
                    } else {
                    ?>
                    <input type="text" id="expired_date_<?php echo $index; ?>" value="<?php if($creditMemoDetail['CreditMemoDetail']['expired_date']!='' && $creditMemoDetail['CreditMemoDetail']['expired_date']!='0000-00-00'){ echo dateShort($creditMemoDetail['CreditMemoDetail']['expired_date']); } else { echo '0000-00-00'; } ?>" name="expired_date[]" style="width:90%; height: 25px; <?php if($creditMemoDetail['Product']['is_expired_date'] == 0){ ?>visibility: hidden;<?php } ?>" class="expired_date <?php if($creditMemoDetail['Product']['is_expired_date'] == 1){ ?>validate[required]<?php } ?>" readonly="readonly" />
                    <?php
                    }
                    ?>
                </div>
            ...
            <td style="width:6%; text-align: center;padding: 0px;">
                <div class="inputContainer" style="width:100%">
                    ...
                    if($isLock == 1){
                    ?>
                    <input type="hidden" name="qty[]" value="<?php echo $creditMemoDetail['CreditMemoDetail']['qty']; ?>" />
                    <input type="text" value="<?php echo number_format($creditMemoDetail['CreditMemoDetail']['qty'], 0); ?>" style="width:70%; height: 25px;" class="qty" disabled="" />
                    <?php
                    } else {
                    ?>
                    <input type="text" id="qty_<?php echo $index; ?>" name="qty[]" value="<?php echo number_format($creditMemoDetail['CreditMemoDetail']['qty'], 0); ?>" style="width:70%; height: 25px;" class="qty interger" />
                    <?php
                    }
                    ?>
                </div>
            ...
            <td style="width:6%; text-align: center;padding: 0px;">
                <div class="inputContainer" style="width:100%">
                    ...
                    if($isLock == 1){
                    ?>
                    <input type="hidden" name="qty_free[]" value="<?php echo $creditMemoDetail['CreditMemoDetail']['qty_free']; ?>" />
                    <input type="text" value="<?php echo number_format($creditMemoDetail['CreditMemoDetail']['qty_free'], 0); ?>" style="width:70%; height: 25px;" class="qty_free" disabled="" />
                    <?php
                    } else {
                    ?>
                    <input type="text" id="qty_free_<?php echo $index; ?>" name="qty_free[]" value="<?php echo number_format($creditMemoDetail['CreditMemoDetail']['qty_free'], 0); ?>" style="width:70%; height: 25px;" class="qty_free interger" />
                    <?php
                    }
                    ?>
                </div>
            ...
            <td style="width:9%; padding: 0px; text-align: center">
                <div class="inputContainer" style="width:100%">
                    <select id="qty_uom_id_<?php echo $index; ?>" style="width:80%; height: 25px;" name="qty_uom_id[]" class="qty_uom_id validate[required]" >
                        <?php
                        $query=mysql_query("SELECT id,name,abbr,1 AS conversion FROM uoms WHERE id=".$creditMemoDetail['Product']['price_uom_id']."
                                            UNION
                                            SELECT id,name,abbr,(SELECT value FROM uom_conversions WHERE is_active=1 AND from_uom_id=".$creditMemoDetail['Product']['price_uom_id']." AND to_uom_id=uoms.id) AS conversion FROM uoms WHERE id IN (SELECT to_uom_id FROM uom_conversions WHERE is_active=1 AND from_uom_id=".$creditMemoDetail['Product']['price_uom_id'].")
                                            ORDER BY conversion ASC");
                        $i = 1;
                        $length = mysql_num_rows($query);
                        while($data=mysql_fetch_array($query)){
                            $selected = "";
                            if($data['id'] == $creditMemoDetail['CreditMemoDetail']['qty_uom_id']){   
                                $selected = ' selected="selected" ';
                            }
                        ?>
                        <option <?php echo $selected; ?>data-sm="<?php if($length == $i){ ?>1<?php }else{ ?>0<?php } ?>" data-item="<?php if($data['id'] == $creditMemoDetail['Product']['price_uom_id']){ echo "first"; }else{ echo "other";} ?>" value="<?php echo $data['id']; ?>" conversion="<?php echo $data['conversion']; ?>"><?php echo $data['name']; ?></option>
                        <?php 
                        $i++;
                        } 
                        ?>
            ...
            <td style="width:9%; padding: 0px; text-align: center">
                <div class="inputContainer" style="width:100%">
                    <input type="text" <?php if(!$allowEditPrice){ ?>readonly="readonly"<?php } ?> value="<?php echo number_format($creditMemoDetail['CreditMemoDetail']['unit_price'], 3); ?>" id="unit_price_<?php echo $index; ?>" name="unit_price[]" style="width:70%; height: 25px;" class="float unit_price" />
                </div>
            </td>
            <td style="width:8%; padding: 0px; text-align: center">
                <div class="inputContainer" style="width:100%;">
                    ...
                    if ($allowProductDiscount) {
                        $disDip = 'display: none';
                        if($creditMemoDetail['CreditMemoDetail']['discount_amount'] > 0){
                            $disDip = '';
                        }
                    ?>
                        <input type="text" class="discount btnDiscountCM float" name="discount[]" id="discount_<?php echo $index; ?>" value="<?php echo number_format($creditMemoDetail['CreditMemoDetail']['discount_amount'], 3); ?>" style="width: 70%; height: 25px;" readonly="readonly" />
                        <img alt="Remove" src="<?php echo $this->webroot . 'img/button/cross.png'; ?>" class="btnRemoveDiscount" align="absmiddle" style="cursor: pointer;<?php echo $disDip; ?>" onmouseover="Tip('Remove')" />
                        <?php
                    }else{
                    ?>
                        <input type="hidden" class="discount btnDiscountCM float" name="discount[]" id="discount_<?php echo $index; ?>" value="<?php echo number_format($creditMemoDetail['CreditMemoDetail']['discount_amount'], 3); ?>" />
                    <?php
                    }
                    ?>
                </div>
            ...
            <td style="width:9%; text-align: center; padding: 0px;">
                <input type="hidden" id="h_total_price_<?php echo $index; ?>" class="h_total_price float" name="h_total_price[]" value="<?php echo $creditMemoDetail['CreditMemoDetail']['total_price']; ?>" />
                <input type="text" <?php if(!$allowEditPrice){ ?>readonly="readonly"<?php } ?> name="total_price[]" id="total_price_<?php echo $index; ?>" value="<?php echo number_format($creditMemoDetail['CreditMemoDetail']['total_price'] - $creditMemoDetail['CreditMemoDetail']['discount_amount'], 3); ?>" style="width:84%; height: 25px;" class="float total_price" />
            </td>
            <td style="width:4%">
                <img alt="Remove" src="<?php echo $this->webroot . 'img/button/cross.png'; ?>" class="btnRemoveCM" id="btnRemoveCM_<?php echo $index; ?>" align="absmiddle" style="cursor: pointer;" onmouseover="Tip('Remove')" />
            </td>
        </tr>
        <?php
            }
        ...
        if(!empty($creditMemoServices)){
            foreach($creditMemoServices AS $creditMemoService){
                ...
                if($creditMemoService['Service']['uom_id'] != ''){
                    ...
                    $uomName = $rowUom[0];
                    $uomVal  = $creditMemoService['Service']['uom_id'];
                }
        ?>
        <tr class="tblCMList">
            <td class="first" style="width:4%; text-align: center;padding: 0px; height: 30px;"><?php echo ++$index; ?></td>
            ...
            <td style="width:18%; text-align: left; padding: 5px;">
                <div class="inputContainer" style="width:100%">
                    <input type="hidden" class="itemIsLock" value="0" />
                    <input type="hidden" name="product_id[]" class="product_id" />
                    <input type="hidden" name="service_id[]" value="<?php echo $creditMemoService['CreditMemoService']['service_id']; ?>" />
                    <input type="hidden" name="discount_id[]" value="<?php echo $creditMemoService['CreditMemoService']['discount_id']; ?>" />
                    <input type="hidden" name="discount_amount[]" value="<?php echo $creditMemoService['CreditMemoService']['discount_amount']; ?>" />
                    <input type="hidden" name="discount_percent[]" value="<?php echo $creditMemoService['CreditMemoService']['discount_percent']; ?>" />
                    <input type="hidden" name="cm_conversion[]" class="cm_conversion" value="0" />
                    <input type="hidden" name="small_val_uom[]" class="small_val_uom" value="0" />
                    <input type="hidden" name="note[]" id="note" readonly="readonly" value="<?php echo $creditMemoService['CreditMemoService']['note']; ?>" class="note" />
                    <input type="text" id="product_<?php echo $index; ?>" readonly="readonly" name="product[]" class="product validate[required]" value="<?php echo str_replace('"', '&quot;', $creditMemoService['Service']['name']); ?>" style="width: 75%; height: 25px;" />
                    <img alt="Note" src="<?php echo $this->webroot . 'img/button/note.png'; ?>" class="noteAddCM" align="absmiddle" style="cursor: pointer;" onmouseover="Tip('Note')" />
                </div>
            </td>
            <td style="width:7%; text-align: center;padding: 0px; <?php if($rowSettingUomDetail[0] == 0){ ?>display: none;<?php } ?>">
                <div class="inputContainer" style="width:100%">
                    <input type="text" id="lots_number_<?php echo $index; ?>" name="lots_number[]" style="width:90%; height: 25px; display: none;" class="lots_number" />
                </div>
            </td>
            <td style="width:10%; text-align: center;padding: 0px;">
                <div class="inputContainer" style="width:100%">
                    <input type="text" id="expired_date_<?php echo $index; ?>" name="expired_date[]" style="width:90%; height: 25px; display: none;" class="expired_date" readonly="readonly" />
                </div>
            </td>
            <td style="width:6%; text-align: center;padding: 0px;">
                <div class="inputContainer" style="width:100%">
                    <input type="text" id="qty_<?php echo $index; ?>" name="qty[]" value="<?php echo number_format($creditMemoService['CreditMemoService']['qty'], 0); ?>" style="width:70%; height: 25px;" class="qty interger" />
                </div>
            </td>
            <td style="width:6%; text-align: center;padding: 0px;">
                <div class="inputContainer" style="width:100%">
                    <input type="text" id="qty_free_<?php echo $index; ?>" name="qty_free[]" value="<?php echo number_format($creditMemoService['CreditMemoService']['qty_free'], 0); ?>" style="width:70%; height: 25px;" class="qty_free interger" />
                </div>
            </td>
            <td style="width:9%; padding: 0px; text-align: center">
                <div class="inputContainer" style="width:100%">
            ...
            <td style="width:9%; padding: 0px; text-align: center">
                <div class="inputContainer" style="width:100%">
                    <input type="text" <?php if(!$allowEditPrice){ ?>readonly="readonly"<?php } ?> value="<?php echo number_format($creditMemoService['CreditMemoService']['unit_price'], 3); ?>" id="unit_price_<?php echo $index; ?>" name="unit_price[]" style="width:70%; height: 25px;" class="float unit_price" />
                </div>
            </td>
            <td style="width:8%; padding: 0px; text-align: center">
                <div class="inputContainer" style="width:100%;">
                    ...
                    if ($allowProductDiscount) {
                        $disDip = 'display: none';
                        if($creditMemoService['CreditMemoService']['discount_amount'] > 0){
                            $disDip = '';
                        }
                    ?>
                        <input type="text" class="discount btnDiscountCM float" name="discount[]" id="discount_<?php echo $index; ?>" value="<?php echo number_format($creditMemoService['CreditMemoService']['discount_amount'], 3); ?>" style="width: 70%; height: 25px;" readonly="readonly" />
                        <img alt="Remove" src="<?php echo $this->webroot . 'img/button/cross.png'; ?>" class="btnRemoveDiscount" align="absmiddle" style="cursor: pointer;<?php echo $disDip; ?>" onmouseover="Tip('Remove')" />
                        <?php
                    }else{
                    ?>
                        <input type="hidden" class="discount btnDiscountCM float" name="discount[]" id="discount_<?php echo $index; ?>" value="<?php echo number_format($creditMemoService['CreditMemoService']['discount_amount'],  2); ?>" />
                    <?php
                    }
                    ?>
                </div>
            ...
            <td style="width:9%; text-align: center; padding: 0px;">
                <input type="hidden" id="h_total_price_<?php echo $index; ?>" class="h_total_price float" name="h_total_price[]" value="<?php echo $creditMemoService['CreditMemoService']['total_price']; ?>" />
                <input type="text" <?php if(!$allowEditPrice){ ?>readonly="readonly"<?php } ?> name="total_price[]" value="<?php echo number_format($creditMemoService['CreditMemoService']['total_price'] - $creditMemoService['CreditMemoService']['discount_amount'], 3); ?>" id="total_price_<?php echo $index; ?>" style="width:84%; height: 25px;" class="float total_price" />
            </td>
            <td style="width:4%">
                <img alt="Remove" src="<?php echo $this->webroot . 'img/button/cross.png'; ?>" class="btnRemoveCM" id="btnRemoveCM_<?php echo $index; ?>" align="absmiddle" style="cursor: pointer;" onmouseover="Tip('Remove')" />
            </td>
        ...
        <tr class="tblCMList">
            ...
            <td style="width:18%; text-align: left; padding: 5px;">
                <div class="inputContainer" style="width:100%">
                    ...
                    <input type="hidden" name="product_id[]" class="product_id" />
                    <input type="hidden" name="service_id[]" />
                    <input type="hidden" name="discount_id[]" value="<?php echo $creditMemoMisc['CreditMemoMisc']['discount_id']; ?>" />
                    <input type="hidden" name="discount_amount[]" value="<?php echo $creditMemoMisc['CreditMemoMisc']['discount_amount']; ?>" />
                    <input type="hidden" name="discount_percent[]" value="<?php echo $creditMemoMisc['CreditMemoMisc']['discount_percent']; ?>" />
                    <input type="hidden" name="cm_conversion[]" class="cm_conversion" value="0" />
                    <input type="hidden" name="small_val_uom[]" class="small_val_uom" value="0" />
                    <input type="hidden" name="note[]" id="note" readonly="readonly" value="<?php echo $creditMemoMisc['CreditMemoMisc']['note']; ?>" class="note" />
                    <input type="text" id="product_<?php echo $index; ?>" readonly="readonly" name="product[]" class="product validate[required]" value="<?php echo str_replace('"', '&quot;', $creditMemoMisc['CreditMemoMisc']['description']); ?>" style="width: 75%; height: 25px;" />
                    <img alt="Note" src="<?php echo $this->webroot . 'img/button/note.png'; ?>" class="noteAddCM" align="absmiddle" style="cursor: pointer;" onmouseover="Tip('Note')" />
                </div>
            </td>
            <td style="width:7%; text-align: center;padding: 0px; <?php if($rowSettingUomDetail[0] == 0){ ?>display: none;<?php } ?>">
            ...
            <td style="width:6%; text-align: center;padding: 0px;">
                <div class="inputContainer" style="width:100%">
                    <input type="text" id="qty_<?php echo $index; ?>" name="qty[]" value="<?php echo number_format($creditMemoMisc['CreditMemoMisc']['qty'], 0); ?>" style="width:70%; height: 25px;" class="qty interger" />
                </div>
            </td>
            <td style="width:6%; text-align: center;padding: 0px;">
                <div class="inputContainer" style="width:100%">
                    <input type="text" id="qty_free_<?php echo $index; ?>" name="qty_free[]" value="<?php echo number_format($creditMemoMisc['CreditMemoMisc']['qty_free'], 0); ?>" style="width:70%; height: 25px;" class="qty_free interger" />
                </div>
            </td>
            <td style="width:9%; padding: 0px; text-align: center">
                <div class="inputContainer" style="width:100%">
                    <select id="qty_uom_id_<?php echo $index; ?>" style="width:80%; height: 25px;" name="qty_uom_id[]" class="qty_uom_id validate[required]" >
                        ...
                        foreach ($uoms as $uom) {
                            $selected = "";
                            if($uom['Uom']['id'] == $creditMemoMisc['CreditMemoMisc']['qty_uom_id']){
                                $selected = 'selected="selected"';
                            }
            ...
            <td style="width:9%; padding: 0px; text-align: center">
                <div class="inputContainer" style="width:100%">
                    <input type="text" <?php if(!$allowEditPrice){ ?>readonly="readonly"<?php } ?> value="<?php echo number_format($creditMemoMisc['CreditMemoMisc']['unit_price'], 3); ?>" id="unit_price_<?php echo $index; ?>" name="unit_price[]" style="width:70%; height: 25px;" class="float unit_price" />
                </div>
            </td>
            <td style="width:8%; padding: 0px; text-align: center">
                <div class="inputContainer" style="width:100%;">
                    ...
                    if ($allowProductDiscount) {
                        $disDip = 'display: none';
                        if($creditMemoMisc['CreditMemoMisc']['discount_amount'] > 0){
                            $disDip = '';
                        }
                    ?>
                        <input type="text" class="discount btnDiscountCM float" name="discount[]" id="discount_<?php echo $index; ?>" value="<?php echo number_format($creditMemoMisc['CreditMemoMisc']['discount_amount'], 3); ?>" style="width: 70%; height: 25px;" readonly="readonly" />
                        <img alt="Remove" src="<?php echo $this->webroot . 'img/button/cross.png'; ?>" class="btnRemoveDiscount" align="absmiddle" style="cursor: pointer;<?php echo $disDip; ?>" onmouseover="Tip('Remove')" />
                        <?php
                    }else{
                    ?>
                        <input type="hidden" class="discount btnDiscountCM float" name="discount[]" id="discount_<?php echo $index; ?>" value="<?php echo number_format($creditMemoMisc['CreditMemoMisc']['discount_amount'], 3); ?>" />
                    <?php
                    }
                    ?>
                </div>
            ...
            <td style="width:9%; text-align: center; padding: 0px;">
                <input type="hidden" id="h_total_price_<?php echo $index; ?>" class="h_total_price float" name="h_total_price[]" value="<?php echo $creditMemoMisc['CreditMemoMisc']['total_price']; ?>" />
                <input type="text" <?php if(!$allowEditPrice){ ?>readonly="readonly"<?php } ?> name="total_price[]" value="<?php echo number_format($creditMemoMisc['CreditMemoMisc']['total_price'] - $creditMemoMisc['CreditMemoMisc']['discount_amount'], 3); ?>" id="total_price_<?php echo $index; ?>" style="width:84%; height: 25px;" class="float total_price" />
            </td>
            <td style="width:4%">
                <img alt="Remove" src="<?php echo $this->webroot . 'img/button/cross.png'; ?>" class="btnRemoveCM" id="btnRemoveCM_<?php echo $index; ?>" align="absmiddle" style="cursor: pointer;" onmouseover="Tip('Remove')" />
            </td>
