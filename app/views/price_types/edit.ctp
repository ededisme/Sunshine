...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("#PriceTypeEditForm").validationEngine('attach', {
            isOverflown: true,
            overflownDIV: ".ui-tabs-panel"
        });
        $("#PriceTypeEditForm").ajaxForm({
            ...
            success: function(result) {
                ...
                if(result != '<?php echo MESSAGE_DATA_HAS_BEEN_SAVED; ?>' && result != '<?php echo MESSAGE_DATA_COULD_NOT_BE_SAVED; ?>' && result != '<?php echo MESSAGE_DATA_ALREADY_EXISTS_IN_THE_SYSTEM; ?>'){
                    createSysAct('Price Type', 'Edit', 2, result);
                    $("#dialog").html('<p><span class="ui-icon ui-icon-info" style="float:left; margin:0 7px 20px 0;"></span><?php echo MESSAGE_PROBLEM; ?></p>');
                }else {
                    createSysAct('Price Type', 'Edit', 1, '');
                    // alert message
    ...
    <table>
            ...
            <td colspan="3">
                <div class="inputContainer" style="width: 100%;">
                    <select style="width: 140px;" id="PriceTypeSetTo" class="validate[required]" name="data[PriceType][is_set]">
                        ...
                        <option value="1" <?php if($this->data['PriceType']['is_set'] == 1){ ?>selected="selected"<?php } ?>><?php echo TABLE_AS_AMOUNT; ?></option>
                        <option value="2" <?php if($this->data['PriceType']['is_set'] == 2){ ?>selected="selected"<?php } ?>><?php echo TABLE_AS_PERCENT; ?></option>
                        <option value="3" <?php if($this->data['PriceType']['is_set'] == 3){ ?>selected="selected"<?php } ?>><?php echo TABLE_AS_ADD_ON; ?></option>
                    </select>
                </div>
            </td>
        </tr>
        <tr>
            ...
            <td colspan="3">
                <div class="inputContainer" style="width: 100%;">
                    <?php
                    $sqlApply = mysql_query("SELECT price_type_id FROM pos_price_types WHERE company_id IN (SELECT company_id FROM price_type_companies WHERE price_type_id = ".$this->data['PriceType']['id'].")  AND price_type_id = ".$this->data['PriceType']['id']." AND is_active = 1 LIMIT 1");
                    ?>
                    <select name="apply_to" style="width: 140px;" id="PriceTypeApplyTo">
                        <option value="0"><?php echo INPUT_SELECT; ?></option>
                        <option value="1" <?php if(mysql_num_rows($sqlApply)){ ?>selected="selected"<?php } ?>><?php echo MENU_POS; ?></option>
                    </select>
                </div>
            </td>
        </tr>
