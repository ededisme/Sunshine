 ...
 * export to excel
 */
$filename="public/report/employee_balance_" . $user['User']['id'] . ".csv";
$fp=fopen($filename,"wb");
$excelContent = MENU_REPORT_EMPLOYEE_BALANCE . "\n\n";
if($data[1]!='') {
...
$excelContent .= "\n\n".TABLE_NO."\t".TABLE_TYPE."\t".TABLE_DATE."\t".TABLE_COMPANY."\t".TABLE_REFERENCE."\t".TABLE_INVOICE_CODE."\t".TABLE_ACCOUNT."\t".GENERAL_AMOUNT."\t".GENERAL_BALANCE;
...
$aColumns = array(
    'gl.id',
    '(SELECT name FROM employees WHERE id=gld.employee_id)',
    'type',
    'date',
    '(SELECT name FROM companies WHERE id=gld.company_id)',
    'reference',
...
if (isset($_GET['iSortCol_0'])) {
    ...
    if ($sOrder == "ORDER BY") {
        $sOrder = 'ORDER BY (SELECT name FROM employees WHERE id=gld.employee_id) ASC, date ASC';
    }
}
...
/* Customize condition */
$condition = "gl.is_approve=1 AND gl.is_active=1 AND gld.employee_id IS NOT NULL AND gld.employee_id!=''";
...
if ($data[4] != '') {
    $condition != '' ? $condition .= ' AND ' : '';
    $condition .= 'gld.employee_id IN (SELECT employee_id FROM employee_egroups WHERE egroup_id=' . $data[4] . ')';
}
if ($data[5] != '') {
    $condition != '' ? $condition .= ' AND ' : '';
    $condition .= 'gld.employee_id=' . $data[5];
}
if ($data[6] != '') {
    $condition != '' ? $condition .= ' AND ' : '';
