...
$aColumns = array("quotations.id", 
    ...
    "customers.name",
    "IFNULL(quotations.total_amount,0) - IFNULL(quotations.discount,0)",
    "IFNULL(quotations.total_vat,0)",
    "IF(quotations.is_approve=0,'Not Yet','Ready')",
    "quotations.status",
...
if ($data[1] != '') {
    ...
    $condition .= '"' . dateConvert(str_replace("|||", "/", $data[1])) . '" <= DATE(quotations.quotation_date)';
}
if ($data[2] != '') {
    $condition != '' ? $condition .= ' AND ' : '';
    $condition .= '"' . dateConvert(str_replace("|||", "/", $data[2])) . '" >= DATE(quotations.quotation_date)';
}
$condition != '' ? $condition .= ' AND ' : '';
if ($data[3] == '') {
    $condition .= 'quotations.status!=0';
} else {
    $condition .= 'quotations.status=' . $data[3];
}
$condition != '' ? $condition .= ' AND ' : '';
if ($data[4] != '') {
...
while ($aRow = mysql_fetch_array($rResult)) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        } else if ($aColumns[$i] == 'quotations.status') {
            switch ($aRow[$i]) {
                case 0:
                    $row[] = 'Void';
                    break;
