 ...
 * export to excel
 */
$filename="public/report/customer_balance_" . $user['User']['id'] . ".csv";
$fp=fopen($filename,"wb");
$excelContent = MENU_REPORT_CUSTOMER_BALANCE . "\n\n";
if($data[1]!='') {
...
$aColumns = array(
    'gl.id',
    '(SELECT name FROM customers WHERE id=gld.customer_id)',
    'type',
    'date',
...
if (isset($_GET['iSortCol_0'])) {
    ...
    if ($sOrder == "ORDER BY") {
        $sOrder = 'ORDER BY (SELECT name FROM customers WHERE id=gld.customer_id) ASC, date ASC';
    }
}
...
/* Customize condition */
$condition = "gl.is_approve=1 AND gl.is_active=1 AND gld.customer_id IS NOT NULL AND gld.customer_id !=''";
...
if ($data[4] != '') {
    $condition != '' ? $condition .= ' AND ' : '';
    $condition .= 'gld.customer_id IN (SELECT customer_id FROM customer_cgroups WHERE cgroup_id=' . $data[4] . ')';
}
if ($data[5] != '') {
    $condition != '' ? $condition .= ' AND ' : '';
