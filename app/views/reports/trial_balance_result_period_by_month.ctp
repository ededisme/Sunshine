 ...
 * export to excel
 */
$filename="public/report/trial_balance_period_by_month.csv";
...
<script type="text/javascript">
    $(document).ready(function(){
            ...
            if(!isNaN($(this).text())){
                ...
                if(chart_account_id && year && month){
                    ...
                    $(this).click(function(){
                        ...
                        $("#tabs").tabs("add", "<?php echo $this->base; ?>/general_ledgers/indexByTb/period/" + chart_account_id + "/" + year + "/" + month + "/" + day, "<?php echo MENU_JOURNAL_ENTRY_MANAGEMENT; ?>");
                    });
                }
            }
        });
        ...
        $("#<?php echo $btnExport; ?>").click(function(){
            window.open("<?php echo $this->webroot; ?>public/report/trial_balance_period_by_month.csv", "_blank");
        });
    });
</script>
        ...
        <div id="<?php echo $originTable; ?>">
            <table class="table_report">
                <tr>
                    ...
                    $sql.=",IFNULL((SELECT SUM(debit) FROM general_ledgers gl INNER JOIN general_ledger_details gld ON gl.id=gld.general_ledger_id WHERE gld.chart_account_id=c.id AND gl.is_approve=1 AND gl.is_active=1 AND company_id IN (".$companyId.") AND branch_id IN (".$branchId.") ".($_POST['customer_id']!=''?'AND customer_id='.$_POST['customer_id']:'')." ".($_POST['vendor_id']!=''?'AND vendor_id='.$_POST['vendor_id']:'')." ".($_POST['other_id']!=''?'AND other_id='.$_POST['other_id']:'')." ".($_POST['class_id']!=''?'AND class_id='.$_POST['class_id']:'')." AND MONTH(date)=".$month." AND YEAR(date)=".$year.")-(SELECT SUM(credit) FROM general_ledgers gl INNER JOIN general_ledger_details gld ON gl.id=gld.general_ledger_id WHERE gld.chart_account_id=c.id AND gl.is_approve=1 AND gl.is_active=1 AND company_id IN (".$companyId.") AND branch_id IN (".$branchId.") ".($_POST['customer_id']!=''?'AND customer_id='.$_POST['customer_id']:'')." ".($_POST['vendor_id']!=''?'AND vendor_id='.$_POST['vendor_id']:'')." ".($_POST['other_id']!=''?'AND other_id='.$_POST['other_id']:'')." ".($_POST['class_id']!=''?'AND class_id='.$_POST['class_id']:'')." AND MONTH(date)=".$month." AND YEAR(date)=".$year."),0)";
                    ...
                    ?>
                    <th colspan="2" style="text-align: center;"><?php echo TABLE_TOTAL; ?></th>
                </tr>
                <tr>
                    <?php
                    for($i=4;$i<=$daysInMonth+4;$i++){
                        if($i==4){
