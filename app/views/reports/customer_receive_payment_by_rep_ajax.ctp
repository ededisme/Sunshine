 ...
 * export to excel
 */
$filename="public/report/customer_receive_payment_by_rep_" . $user['User']['id'] . ".csv";
$fp=fopen($filename,"wb");
$excelContent = MENU_REPORT_RECEIVE_PAYMENTS_BY_REP . "\n\n";
if($data[1]!='') {
...
$excelContent .= "\n\n".TABLE_NO."\t".TABLE_CUSTOMER."\t".TABLE_TYPE."\t".TABLE_DATE."\t".TABLE_COMPANY."\t".TABLE_REFERENCE."\t".TABLE_ACCOUNT."\t".GENERAL_AMOUNT;
...
$aColumns = array(
    'gl.id',
    '(SELECT name FROM cgroups WHERE id=(SELECT cgroup_id FROM customer_cgroups WHERE customer_id=gld.customer_id LIMIT 1))',
...
$condition .= " AND type='Receive Payment'";
if ($data[1] != '') {
