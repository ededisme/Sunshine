...
$aColumns = array("sales_orders.id",
    "CONCAT_WS(' ', employees.employee_code, ' - ', employees.name)",
...
/* DB table to use */
$sTable = " sales_orders INNER JOIN customers ON customers.id = sales_orders.customer_id INNER JOIN employees ON employees.id = sales_orders.sales_rep_id INNER JOIN currency_centers ON currency_centers.id = sales_orders.currency_center_id";
...
/* Customize condition */
$condition = 'sales_orders.is_pos IN (0,2)';
if ($data[1] != '') {
...
if ($data[7] != '') {
    $condition != '' ? $condition .= ' AND ' : '';
    $condition .= 'sales_orders.customer_id IN (SELECT customer_id FROM customer_cgroups WHERE cgroup_id=' . $data[7] . ')';
}
if ($data[8] != '') {
    $condition != '' ? $condition .= ' AND ' : '';
    $condition .= 'sales_orders.customer_id=' . $data[8];
}
if ($data[9] != '') {
    $condition != '' ? $condition .= ' AND ' : '';
    $condition .= 'sales_orders.sales_rep_id = ' . $data[9];
}
if ($data[10] != '') {
    $condition != '' ? $condition .= ' AND ' : '';
    $condition .= 'sales_orders.created_by = ' . $data[10];
}
...
while ($aRow = mysql_fetch_array($rResult)) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        } else if ($aColumns[$i] == "CONCAT_WS(' ', customers.customer_code, ' - ', customers.name)") {
            if (trim($aRow[$i]) == '') {
