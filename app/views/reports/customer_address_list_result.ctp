...
<div id="<?php echo $printArea; ?>">
    <?php
    $msg = '<b style="font-size: 18px;">' . MENU_REPORT . '<br />' . MENU_REPORT_CUSTOMER_ADDRESS_LIST . '</b><br /><br />';
    if ($_POST['date_from'] != '') {
    ...
    $sql = "    SELECT Customer.id, Customer.customer_code, Customer.name, Customer.address,
                    Customer.main_number, Customer.mobile_number, Customer.other_number, Customer.email
        ...
        <table class="table" cellspacing="0" style="height: 200px;">
            <tr>
                <th class="first" style="height: 30%;">
                    <?php echo "{$data['customer_code']} <br />{$data['name']}" ?>
                </th>
            </tr>
            <tr>
                <td class="first" style="vertical-align: top;">
                    <?php echo "{$data['address']}<br />Tel: {$data['main_number']}<br />Email: {$data['email']}" ?>
                </td>
            </tr>
        </table>
    </div>
...
<div style="float:right" class="noprint">
    ...
    <select id="<?php echo $numberShow; ?>" style="font-size:14px">
