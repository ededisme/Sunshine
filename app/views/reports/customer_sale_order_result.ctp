...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        <?php echo $oTable; ?> = $("#<?php echo $tblName; ?>").dataTable({
            ...
            "bProcessing": true,
            "bServerSide": true,
            "sAjaxSource": "<?php echo $this->base.'/'.$this->params['controller']; ?>/customerSaleOrderAjax/<?php echo str_replace("/", "|||", implode(',', $_POST)); ?>",
            ...
            "fnInfoCallback": function( oSettings, iStart, iEnd, iMax, iTotal, sPre ) {
                ...
                $("#<?php echo $tblName; ?> tr:gt(0)").each(function(){
                    sub += replaceNum($(this).find(".btnPrintCustomerSaleOrder").attr("sub"));
                    vat += replaceNum($(this).find(".btnPrintCustomerSaleOrder").attr("vat"));
                    total += replaceNum($(this).find(".btnPrintCustomerSaleOrder").attr("total"));
                });
                $('#<?php echo $tblName; ?> > tbody:last').append('<tr><td class="first" style="font-weight: bold;" colspan="4"><?php echo TABLE_TOTAL; ?>:</td><td class="formatCurrency" style="text-align: right;font-weight: bold;"></td><td class="formatCurrency" style="text-align: right;font-weight: bold;">' + (sub) + '</td><td class="formatCurrency" style="text-align: right;font-weight: bold;">' + (vat) + '</td><td class="formatCurrency" style="text-align: right;font-weight: bold;">' + (total) + '</td><td colspan="3"></td></tr>');
                $('.formatCurrency').formatCurrency({colorize:true});
                
                $(".btnPrintCustomerSaleOrder").click(function(event){
                    event.preventDefault();
                    $.ajax({
                        type: "POST",
                        url: "<?php echo $this->base; ?>/orders/printInvoice/" + $(this).attr("rel"),
                        beforeSend: function(){
...
<div id="<?php echo $printArea; ?>">
    <?php
    $msg = '<b style="font-size: 18px;">' . MENU_ORDER . '</b><br /><br />';
    if($_POST['date_from']!='') {
        ...
        <table id="<?php echo $tblName; ?>" class="table_report">
            <thead>
                <tr>
                    ...
                    <th style="width: 140px !important;"><?php echo TABLE_SUB_TOTAL; ?></th>
                    <th style="width: 80px !important;"><?php echo TABLE_VAT; ?></th>
