 ...
 * export to excel
 */
$filename="public/report/sales_by_customer_detail_" . $this->Session->id(session_id()) . ".csv";
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("#<?php echo $btnExport; ?>").click(function(){
            window.open("<?php echo $this->webroot; ?>public/report/sales_by_customer_detail_<?php echo $this->Session->id(session_id()); ?>.csv", "_blank");
        });
    });
</script>
<div id="<?php echo $printArea; ?>">
    ...
    $excelContent .= TABLE_NO."\t".TABLE_TYPE."\t".TABLE_DATE."\t".TABLE_INVOICE_CODE."\t".TABLE_PRODUCT_NAME."\t".TABLE_LOCATION_GROUP."\t".TABLE_QTY."\t".TABLE_UOM."\t".TABLE_PRICE."\t".GENERAL_AMOUNT;
    ?>
    <table id="<?php echo $tblName; ?>" class="table_report">
        ...
        while($data=mysql_fetch_array($query)){
            ...
            if($data['customer_id']!=$oldCustomerId){ 
                if($oldCustomerName!=''){ 
                    $excelContent .= "\n".'Total '.$oldCustomerName."\t\t\t\t\t\t\t\t\t".number_format($subTotalAmount, 2); ?>
            <tr style="font-weight: bold;">
                <td class="first" colspan="9">Total <?php echo $oldCustomerName; ?></td>
            ...
            <?php
                ...
                $excelContent .= "\n".$data['customer_name']; ?>
            <tr>
                <td class="first" colspan="10" style="font-weight: bold;"><?php echo $data['customer_name']; ?></td>
            </tr>
            <?php 
            } 
            $excelContent .= "\n".$index."\t".$data['trans_type']."\t".dateShort($data['order_date'])."\t".$data['code']."\t".$data['product_name']."\t".$data['location_name']."\t".$data['qty']."\t".$data['qty_uom_name']."\t".$data['unit_price']."\t".number_format(($data['qty'] * $data['unit_price']), 2); ?>
            <tr>
        ...
        if($col[7] == '' && $col[8] == '' && $col[9] == ''){ 
            ...
            $query=mysql_query("SELECT
                                    ...
                                    (SELECT name FROM location_groups WHERE id=location_group_id) AS location_name,
                                    ".($col[10]!=''?($col[10]=='1'?'qty_free':'qty'):'(qty + qty_free)')." AS qty,
            ...
            while($data=mysql_fetch_array($query)){
                ...
                $excelContent .= "\n".$index."\t".$data['trans_type']."\t".dateShort($data['order_date'])."\t".$data['code']."\t".$data['service_name']."\t".$data['location_name']."\t".$data['qty']."\t".$data['qty_uom_name']."\t".$data['unit_price']."\t".number_format(($data['qty'] * $data['unit_price']), 2); ?>
                <tr>
                    <td class="first"><?php echo $index++; ?></td>
                    <td><?php echo $data['trans_type']; ?></td>
                    <td><?php echo dateShort($data['order_date']); ?></td>
            ...
            $query=mysql_query("SELECT
                                ...
                                WHERE " . $condition . ($col[10] != ''? ($col[10]=='1' ? ' AND sales_order_miscs.qty_free > 0':' AND sales_order_miscs.qty > 0') : '') . "
                                UNION ALL
                                SELECT
                                    credit_memos.id,
                                    'Credit Memo' AS trans_type,
                                ...
                                WHERE " . str_replace('is_pos=0 AND','',$condition) 
                                        . ($col[10] != ''? ($col[10]=='1' ? ' AND credit_memo_miscs.qty_free > 0':' AND credit_memo_miscs.qty > 0') : '')
                                        . "
                                ORDER BY customer_name,order_date");
            while($data=mysql_fetch_array($query)){
                $arrCode[] = $data['code'];
                ...
                <?php
                    ...
                    $subTotalAmount=0;
                    $excelContent .= "\n".$data['customer_name']; ?>
                <tr><td class="first" colspan="10" style="font-weight: bold;"><?php echo $data['customer_name']; ?></td></tr>
                <?php 
                }
                $excelContent .= "\n".$index."\t".$data['trans_type']."\t".$data['order_date']."\t".$data['code']."\t".$data['description']."\t".$data['location_name']."\t".$data['qty']."\t".$data['qty_uom_name']."\t".$data['unit_price']."\t".number_format(($data['qty'] * $data['unit_price']), 2); ?>
                <tr>
                    <td class="first"><?php echo $index++; ?></td>
                    <td><?php echo $data['trans_type']; ?></td>
