...
$excelContent = MENU_REFERRAL_REPORT . "\n\n";
...
$excelContent .= "\n\n".TABLE_NO."\t".TABLE_DATE."\t".TABLE_INVOICE_CODE."\t".TABLE_CUSTOMER_NUMBER."\t".TABLE_CUSTOMER_NAME."\t".DOCTOR_DOCTOR."\t".SECTION_SECTION."\t".SERVICE_SERVICE."\t".TABLE_REFERRAL."\t".TABLE_QTY."\t".GENERAL_UNIT_PRICE."\t".GENERAL_DISCOUNT."($)\t".TABLE_TOTAL_AMOUNT."($)";
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 * Easy set variables
 */
...
/* DB table to use */
$sTable = "invoice_details INNER JOIN invoices ON invoices.id = invoice_details.invoice_id 
INNER JOIN services ON services.id = invoice_details.service_id 
INNER JOIN sections on sections.id = services.section_id 
INNER JOIN queues ON queues.id = invoices.queue_id 
INNER JOIN patients ON patients.id=queues.patient_id
LEFT JOIN referrals ON referrals.id=patients.referral_id";
...
if ($data[3] != '') {
    $condition != '' ? $condition .= ' AND ' : '';
    $condition .= 'services.id=' . $data[3];
}
if ($data[4] != '') {
    $condition != '' ? $condition .= ' AND ' : '';
    $condition .= 'patients.referral_id=' . $data[4];
}
