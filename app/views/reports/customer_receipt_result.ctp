...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        <?php echo $oTable; ?> = $("#<?php echo $tblName; ?>").dataTable({
            ...
            "bProcessing": true,
            "bServerSide": true,
            "sAjaxSource": "<?php echo $this->base.'/'.$this->params['controller']; ?>/customerReceiptAjax/<?php echo str_replace("/", "|||", implode(',', $_POST)); ?>",
            ...
            "fnInfoCallback": function( oSettings, iStart, iEnd, iMax, iTotal, sPre ) {
                ...
                $(".btnPrintCustomerInvoice").click(function(event){
                    ...
                    var url = "<?php echo $this->base . '/cashiers'; ?>/printInvoice/"+$(this).attr("rel");
                    $.ajax({
                        type: "POST",
                        url: url,
                        beforeSend: function(){
                ...
                $(".btnPrintCustomerReceipt").click(function(event){
                    event.preventDefault();
                    var url = "<?php echo $this->base . '/cashiers'; ?>/printInvoiceReceipt/"+$(this).attr("rel");
                    $.ajax({
                        type: "POST",
                        url: url,
                        beforeSend: function(){
                ...
                $(".btnPrintInvoiceReportPos").click(function(event){
                    event.preventDefault();
                ...
                $(".btnPrintInvoiceReportDeposite").click(function(event){
                    event.preventDefault();
                    var url = "<?php echo $this->base . '/deposits'; ?>/printDepositPayment/"+$(this).attr("rel");
                    $.ajax({
                        type: "POST",
                        url: url,
                        beforeSend: function(){
                ...
                $("#<?php echo $tblName; ?> tr:gt(0)").each(function(){
                    ...
                    totalPaid += Number($(this).find("td:eq(6)").text().replace(/,/g, ""));
                    balance += Number($(this).find("td:eq(7)").text().replace(/,/g, ""));
                });
                $('#<?php echo $tblName; ?> > tbody:last').append('<tr><td class="first" style="font-weight: bold;" colspan="5"><?php echo TABLE_TOTAL; ?>:</td><td class="formatCurrency" style="text-align: right;font-weight: bold;">' + (total) + '</td><td class="formatCurrency" style="text-align: right;font-weight: bold;">' + (totalPaid) + '</td><td class="formatCurrency" style="text-align: right;font-weight: bold;">' + (balance) + '</td><td></td></tr>');
...
<div id="<?php echo $printArea; ?>">
    <?php
    $msg = '<b style="font-size: 18px;">' . MENU_REPORT_RECEIPT . '</b><br /><br />';
    if($_POST['date_from']!='') {
        ...
        <table id="<?php echo $tblName; ?>" class="table_report">
            <thead>
                <tr>
                    ...
                    <th style="width: 120px !important;"><?php echo TABLE_RECEIPT_DATE; ?></th>
                    <th style="width: 120px !important;"><?php echo TABLE_INVOICE_CODE; ?></th>
                    <th style="width: 120px !important;"><?php echo TABLE_RECEIPT_CODE; ?></th>
                    <th><?php echo PATIENT_NAME; ?></th>
                    <th style="width: 140px !important;"><?php echo TABLE_TOTAL_AMOUNT; ?> ($)</th>
                    <th style="width: 140px !important;"><?php echo TABLE_TOTAL_AMOUNT_PAID; ?> ($)</th>
