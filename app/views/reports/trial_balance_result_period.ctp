 ...
 * export to excel
 */
$filename="public/report/trial_balance_period.csv";
    ...
    $(document).ready(function(){
        ...
        $(".link2gl").each(function(){
            if(!isNaN($(this).text())){
                ...
                ?>
                var dateFrom=$(this).parent().parent().find("tr:first th:eq("+parentIndex+")").attr("dateFrom");
                var dateTo=$(this).parent().parent().find("tr:first th:eq("+parentIndex+")").attr("dateTo");
                ...
                if(chart_account_id && dateFrom && dateTo){
                    ...
                    $(this).click(function(){
                        ...
                        $("#tabs").tabs("add", "<?php echo $this->base; ?>/general_ledgers/indexByTbDateRange/" + chart_account_id + "/" + dateFrom + "/" + dateTo + "/" + companyId + "/" + customerId + "/" + vendorId + "/" + otherId + "/" + classId, "<?php echo MENU_JOURNAL_ENTRY_MANAGEMENT; ?>");
                    });
                }
                <?php
                }
        ...
        $("#<?php echo $btnExport; ?>").click(function(){
            window.open("<?php echo $this->webroot; ?>public/report/trial_balance_period.csv", "_blank");
        });
        
        $(".btnTBByMonth").click(function(event){
            event.preventDefault();
            var container=$(this).parent().parent().parent().parent().parent().parent().parent().parent().attr("id");
            var month=$(this).attr("month");
            var year=$(this).attr("year");
            $.ajax({
                type: "POST",
                url: "<?php echo $this->base; ?>/<?php echo $this->params['controller']; ?>/trialBalanceResultPeriodByMonth",
                data: "month=" + month + "&year=" + year + "&company_id=<?php echo $_POST['company_id']; ?>" + "&customer_id=<?php echo $_POST['customer_id']; ?>" + "&vendor_id=<?php echo $_POST['vendor_id']; ?>" + "&other_id=<?php echo $_POST['other_id']; ?>" + "&class_id=<?php echo $_POST['class_id']; ?>",
                beforeSend: function(){
        ...
        <div id="<?php echo $originTable; ?>">
            <table class="table_report">
                <tr>
                    ...
                    ?>
                    <th colspan="2" style="text-align: center;" dateFrom="<?php echo $year; ?>-<?php echo $month; ?>-<?php echo $bDay; ?>" dateTo="<?php echo $year; ?>-<?php echo $month; ?>-<?php echo $daysInMonth; ?>">
                        <?php
                        echo '<a href="" class="btnTBByMonth" style="font: 11px Kh-Battambang,Arial,Helvetica,sans-serif;" month="' . $month . '" year="' . $year . '">' . $monthName[$month-1] . '/' . $year . '</a>';
                    ...
                    $sql.=",IFNULL((SELECT SUM(debit) FROM general_ledgers gl INNER JOIN general_ledger_details gld ON gl.id=gld.general_ledger_id WHERE gld.chart_account_id=c.id AND gl.is_approve=1 AND gl.is_active=1 AND company_id IN (".$companyId.") AND branch_id IN (".$branchId.") ".($_POST['customer_id']!=''?'AND customer_id='.$_POST['customer_id']:'')." ".($_POST['vendor_id']!=''?'AND vendor_id='.$_POST['vendor_id']:'')." ".($_POST['other_id']!=''?'AND other_id='.$_POST['other_id']:'')." ".($_POST['class_id']!=''?'AND class_id='.$_POST['class_id']:'')." ".$condition.")-(SELECT SUM(credit) FROM general_ledgers gl INNER JOIN general_ledger_details gld ON gl.id=gld.general_ledger_id WHERE gld.chart_account_id=c.id AND gl.is_approve=1 AND gl.is_active=1 AND company_id IN (".$companyId.") AND branch_id IN (".$branchId.") ".($_POST['customer_id']!=''?'AND customer_id='.$_POST['customer_id']:'')." ".($_POST['vendor_id']!=''?'AND vendor_id='.$_POST['vendor_id']:'')." ".($_POST['other_id']!=''?'AND other_id='.$_POST['other_id']:'')." ".($_POST['class_id']!=''?'AND class_id='.$_POST['class_id']:'')." ".$condition."),0)";
                    ...
                    ?>
                    <th colspan="2" style="text-align: center;" dateFrom="<?php echo $d1['year']; ?>-<?php echo str_pad($d1['month'], 2, '0', STR_PAD_LEFT); ?>-<?php echo str_pad($d1['day'], 2, '0', STR_PAD_LEFT); ?>" dateTo="<?php echo $d2['year']; ?>-<?php echo str_pad($d2['month'], 2, '0', STR_PAD_LEFT); ?>-<?php echo str_pad($d2['day'], 2, '0', STR_PAD_LEFT); ?>"><?php echo TABLE_TOTAL; ?></th>
                </tr>
                <tr>
                    <?php
                    for($i=4;$i<=$count+4;$i++){
