...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        <?php echo $oTable; ?> = $("#<?php echo $tblName; ?>").dataTable({
            "bProcessing": true,
            "bServerSide": true,
            "sAjaxSource": "<?php echo $this->base.'/'.$this->params['controller']; ?>/productPriceAjax/<?php echo str_replace("/", "|||", implode(',', $_POST)); ?>",
        ...
        $("#<?php echo $btnExport; ?>").click(function(){
            window.open("<?php echo $this->webroot; ?>public/report/product_price_list_<?php echo $this->Session->id(session_id()); ?>.csv", "_blank");
        });
    });
</script>
<div id="<?php echo $printArea; ?>">
    <?php
    $msg = '<b style="font-size: 18px;">' . MENU_PRODUCT_PRICE . '</b><br /><br />';
    if($_POST['company_id']!='') {
        $query=mysql_query("SELECT name FROM companies WHERE id=".$_POST['company_id']);
        $data=mysql_fetch_array($query);
        $msg .= TABLE_COMPANY.': '.$data[0].'<br/>';
    }
    if($_POST['branch_id']!='') {
        $query=mysql_query("SELECT name FROM branches WHERE id=".$_POST['branch_id']);
        $data=mysql_fetch_array($query);
        $msg .= MENU_BRANCH.': '.$data[0].'<br/>';
    }
    if($_POST['pgroup_id']!='') {
        $query=mysql_query("SELECT name FROM pgroups WHERE id=".$_POST['pgroup_id']);
        $data=mysql_fetch_array($query);
        $msg .= ' '.MENU_PRODUCT_GROUP_MANAGEMENT.': '.$data[0].'<br/>';
    }
    if($_POST['created_by']!='') {
        ...
        <table id="<?php echo $tblName; ?>" class="table_report">
            <thead>
                <tr>
                    ...
                    <th style="width: 140px !important;"><?php echo TABLE_BARCODE; ?></th>
                    <th><?php echo TABLE_PRODUCT_NAME; ?></th>
                    <th><?php echo TABLE_UOM; ?></th>
                    <?php
                    $sqlPList = mysql_query("SELECT name FROM price_types WHERE is_active = 1 ORDER BY ordering = 1");
                    while($rowPList = mysql_fetch_array($sqlPList)){
                    ?>
                    <th style="width: 150px;"><?php echo $rowPList[0]; ?></th>
                    <?php
                    }
                    ?>
                </tr>
