...
/* DB table to use */
$sTable = "quotations INNER JOIN users ON users.id = quotations.created_by INNER JOIN customers ON customers.id = quotations.customer_id INNER JOIN currency_centers ON currency_centers.id = quotations.currency_center_id LEFT JOIN quotation_details as qd ON qd.quotation_id=quotations.id LEFT JOIN quotation_services as qs ON qs.quotation_id=quotations.id";
...
$sQuery = "
        ...
        FROM   $sTable
        $sWhere
        GROUP BY quotations.quotation_code
        $sOrder
        $sLimit
";
...
while ($aRow = mysql_fetch_array($rResult)) {
    ...
    $output['aaData'][] = $row;
    $queryProduct=  mysql_query("SELECT (SELECT CONCAT_WS(' ',code,name) FROM products WHERE id=product_id) AS product_name,qty,(SELECT name FROM uoms WHERE id=qty_uom_id) as uomName,unit_price,total_price FROM quotation_details as qd INNER JOIN quotations ON quotations.id=qd.quotation_id  WHERE quotation_id=".$aRow[0]." AND ".$condition);
    ...
    if ($data[9] != '') {
        $querySevice=  mysql_query("SELECT (SELECT name FROM services WHERE id=service_id) AS service_name,qty,(SELECT name FROM uoms WHERE id IN (SELECT uom_id FROM services WHERE is_active=1)) as uomName,unit_price,total_price FROM quotation_services as qs INNER JOIN quotations ON quotations.id=qs.quotation_id  WHERE quotation_id=".$aRow[0]);
    }else{
        $querySevice=  mysql_query("SELECT (SELECT name FROM services WHERE id=service_id) AS service_name,qty,(SELECT name FROM uoms WHERE id IN (SELECT uom_id FROM services WHERE is_active=1)) as uomName,unit_price,total_price FROM quotation_services as qs INNER JOIN quotations ON quotations.id=qs.quotation_id  WHERE quotation_id=".$aRow[0]." AND ".$condition);
    }
