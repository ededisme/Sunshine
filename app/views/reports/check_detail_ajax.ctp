 ...
 * export to excel
 */
$filename="public/report/check_" . $user['User']['id'] . ".csv";
$fp=fopen($filename,"wb");
$excelContent = MENU_REPORT_CHECK . "\n\n";
if($data[1]!='') {
...
$excelContent .= "\n\n".TABLE_NO."\t".TABLE_DATE."\t".TABLE_REFERENCE."\t".TABLE_ADJUST."\t".TABLE_COMPANY."\t".TABLE_NAME."\t".TABLE_ACCOUNT."\t".GENERAL_DEBIT."\t".GENERAL_CREDIT;
...
$aColumns = array(
    ...
    'IF(customer_id!="",(SELECT name FROM customers WHERE id=customer_id),IF(vendor_id!="",(SELECT name FROM vendors WHERE id=vendor_id),IF(employee_id!="",(SELECT name FROM employees WHERE id=employee_id),IF(other_id!="",(SELECT name FROM others WHERE id=other_id),""))))',
...
$condition .= " AND (type='Check' OR type='Credit Memo Check')";
if ($data[1] != '') {
...
while ($aRow = mysql_fetch_array($rResult)) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        if ($i == 0) {
            ...
            if ($aRow[0] == $tmpId) {
                $row[] = '<input type="hidden" value="' . $aRow[0] . '" class="link2gl" />';
                $excelContent .= "\n";
            } else {
                $row[] = ++$index . '<input type="hidden" value="' . $aRow[0] . '" class="link2gl" />';
                $excelContent .= "\n" . $index;
            }
