...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $("#UomConversionEditForm").validationEngine();
        $("#UomConversionEditForm").ajaxForm({
            ...
            beforeSubmit: function(arr, $form, options) {
                $('#UomConversionEditForm').validationEngine('hideAll');
                $(".txtSaveUomConversion").html("<?php echo ACTION_LOADING; ?>");
                $(".loader").attr("src", "<?php echo $this->webroot; ?>img/layout/spinner.gif");
            },
            success: function(result) {
                $(".loader").attr("src", "<?php echo $this->webroot; ?>img/layout/spinner-placeholder.gif");
                var rightPanel=$("#UomConversionEditForm").parent();
                ...
                if(result != '<?php echo MESSAGE_DATA_HAS_BEEN_SAVED; ?>' && result != '<?php echo MESSAGE_DATA_COULD_NOT_BE_SAVED; ?>'){
                    createSysAct('UoM Conversion', 'Edit', 2, result);
                    $("#dialog").html('<p><span class="ui-icon ui-icon-info" style="float:left; margin:0 7px 20px 0;"></span><?php echo MESSAGE_PROBLEM; ?></p>');
                }else {
                    createSysAct('UoM Conversion', 'Edit', 1, '');
                    // alert message
    ...
    function hideOptionOtherUom(){
        var otherUomId  = $("#allIdOtherUom").val().split(":");
        var mainIdUom   = $("#oldHideFromUom").val();
        var smallIdUom  = $("#oldHideUomSmall").val();
        var i;
        $("#UomConversionFromUomId").find("option[value='"+mainIdUom+"']").hide();
        $("#UomConversionToUomSmallId").find("option[value='"+smallIdUom+"']").hide();
        for (i = 0; i < otherUomId.length; i++){
            if(otherUomId[i] != ''){
                var newVal = otherUomId[i];
                var id     = otherUomId[i];
                $(".otherUom").each(function(){
                    var objUom = $(this).find(".otherConversionUom");
                    var uomId  = objUom.attr('data-id');
                    objUom.find("option[value='" + mainIdUom + "']").hide();
                    objUom.find("option[value='" + smallIdUom + "']").hide();
                    if(uomId != id){
                        objUom.find("option[value='" + newVal + "']").hide();
                    }
                });
...
<br />
<?php 
echo $this->Form->create('UomConversion');
...
<fieldset>
    ...
    <table>
        <tr>
            ...
            <td colspan="3">
                <input type="hidden" id="oldHideFromUom" value="<?php echo $id; ?>" />
                <?php echo $this->Form->input('from_uom_id', array('empty' => INPUT_SELECT, 'label' => false, 'options' => $uomListMain, 'class' => 'validate[required]', 'selected' => $id)); ?></td>            
        </tr>
        <tr>
            ...
            <td>
                ...
                $symbol = ":";
                $text = "";
                foreach ($uomAddes as $uomAdde) {
                    $text .= $symbol . $uomAdde['UomConversion']['to_uom_id'];
                    if ($uomAdde['UomConversion']['is_small_uom'] == 1) {
                ?>
                        <input type="hidden" id="oldHideUomSmall" value="<?php echo $uomAdde['UomConversion']['to_uom_id']; ?>" /> 
                        <?php echo $this->Form->input('to_uom_id', array('id' => 'UomConversionToUomSmallId', 'empty' => INPUT_SELECT, 'label' => false, 'options' => $uomList, 'class' => 'validate[required]', 'selected' => $uomAdde['UomConversion']['to_uom_id'])); ?></td>
                    <td><label for="UomConversionValueSmall"><?php echo TABLE_VOLUME; ?> <span class="red">*</span> :</label></td>
                    <td><?php echo $this->Form->text('value', array('id' => 'UomConversionValueSmall', 'class' => 'validate[required] float', 'style' => 'width:120px', 'value' => $uomAdde['UomConversion']['value'])); ?></td>
           <?php
                }
            }
            ?>
    ...
    <input type="hidden" value="<?php echo $text; ?>" id="allIdOtherUom" />
    <br/>
    <fieldset>
        ...
        <table id="tblOtherUom">
            <tr>
                <td colspan="5">
                    <div class="buttons">
                        <a href="#" class="positive btnAddNewUomConversionOther" <?php if(sizeof($uomAddes) == 2){ ?>style="display: none;"<?php } ?>>
            ...
            $i = 1;
            foreach ($uomAddes as $uomAdde) {
                if ($uomAdde['UomConversion']['is_small_uom'] == 0) {
                    ?>
                    <tr class="otherUom">
                        <td><?php echo TABLE_OTHER_UOM; ?> <span class="red">*</span> :</td>
                        <td><?php echo $this->Form->input('other_uom_id', array('name' => 'data[other_uom][]', 'id' => 'otherConversionUom_'.$i, 'class' => 'otherConversionUom validate[required]', 'empty' => INPUT_SELECT, 'label' => false, 'options' => $uomList, 'selected' => $uomAdde['UomConversion']['to_uom_id'], 'data-id' => $uomAdde['UomConversion']['to_uom_id'])); ?></td>
                        <td><?php echo TABLE_VOLUME; ?> <span class="red">*</span> :</td>
                        <td>
                            <input type="hidden" class="hideOtherUom" value="<?php echo $uomAdde['UomConversion']['to_uom_id']; ?>" />
                            <?php echo $this->Form->text('value', array('name' => 'data[other_value][]', 'id' => 'UomConversionValueOther_'.$i, 'class' => 'UomConversionValueOther validate[required] float', 'style' => 'width:120px', 'value' => $uomAdde['UomConversion']['value'])); ?></td>
                        <td>
