...
<script type="text/javascript">
	...
	$("#OrderListInvAdj").remove();
        ...
        // Search Product PUC
        $("#InvAdjPUC").keypress(function(e){
            if((e.which && e.which == 13) || e.keyCode == 13){
                if(searchAction == 0){
                    searchAction = 1;
                    searchProduct(2, $(this).val());
                }
                return false;
            }
        });
        ...
        $("#InvAdjSKU, #InvAdjPUC").focus(function(){
    ...
    function getTotalQtyOnHand(obj){
        ...
        if(location != '' && productId != '' && adjDate != ''){
            ...
            $.ajax({
                ...
                data:   "product_id="+productId+"&inv_adj_id=<?php echo $cycleProductId; ?>",
                beforeSend: function(arr, $form, options) {
                    $(".loader").attr('src','<?php echo $this->webroot; ?>img/layout/spinner.gif');
...
<div class="inputContainer" style="width:100%; float: left;" id="searchFormInvAdj">
    <table style="width: 100%;">
        <tr>
            ...
            <td>
                <img alt="Search" align="absmiddle" style="cursor: pointer; width: 22px; height: 22px;" class="searchProductInvAdj" onmouseover="Tip('<?php echo GENERAL_SEARCH; ?>')" src="<?php echo $this->webroot . 'img/button/search.png'; ?>" />
            </td>
        </tr>
    </table>
</div>
...
<div id="bodyListInvAdj">
    <table id="tblInvAdj" class="table" cellspacing="0" style="padding: 0px; width:100%">
        ...
        <?php
        $index = 0;
        $sqlDetail = mysql_query("SELECT * FROM cycle_product_details WHERE cycle_product_id = ".$cycleProductId);
        ...
        <tr class="tblInvAdjList">
            ...
            <td style="width: 16%;">
                <div class="inputContainer" style="width:100%">
                    ...
                    <input type="text" style="width: 90%;" id="productname_<?php echo $index; ?>" value="<?php echo str_replace('"', '&quot;', $rowProduct['name']); ?>" class="product_name" readonly="readonly" />
                </div>
            </td>
            <td style="width: 9%; <?php if($rowSettingUomDetail[0] == 0){ ?>display: none;<?php } ?>">
                <div class="inputContainer" style="width:100%">
            ...
            $totalOrder = 0;
            $sqlOrder = mysql_query("SELECT sum(sor.qty) as total_order FROM `stock_orders` as sor WHERE sor.cycle_product_id = ".$cycleProduct['CycleProduct']['id']." AND sor.product_id = ".$rowDetail['product_id']." AND sor.location_group_id = ".$cycleProduct['CycleProduct']['location_group_id']." AND location_id = ".$rowDetail['location_id']." AND lots_number = '{$rowDetail['lots_number']}' AND expired_date = '{$rowDetail['expired_date']}' AND date <= '".$cycleProduct['CycleProduct']['date']."' GROUP BY sor.product_id");
            ...
            $totalNewDisplay  = isset($rowDetail['qty_difference']) ? $totalQty + $rowDetail['qty_difference'] : 0;
            $totalDiffDisplay = isset($rowDetail['qty_difference']) ? $rowDetail['qty_difference'] : 0;
            $displayNew  = showTotalQty($totalNewDisplay, $rowProduct['uom_abbr'], $rowProduct['small_val_uom'], $smallUomLabel);
            $displayDiff = showTotalQty($totalDiffDisplay, $rowProduct['uom_abbr'], $rowProduct['small_val_uom'], $smallUomLabel);
            ?>
            <td style="width: 9%;">
                <div class="inputContainer" style="width:100%">
                    <input type="hidden" class="invAdjNewQtyHidden" name="new_qty[]" value="<?php echo $totalNewDisplay; ?>" />
                    <input type="text" class="newQtyLabel" id="newQtyLabel_<?php echo $index; ?>" value="<?php echo $displayNew; ?>" readonly="readonly" style="width: 90%; height: 25px;" />
                </div>
            </td>
            <td style="width: 9%;">
                <div class="inputContainer" style="width:100%">
                    <input type="hidden" class="invAdjQtyDifferenceHidden" name="qty_difference[]" value="<?php echo $totalDiffDisplay; ?>" />
                    <input type="text" class="InvAdjQtyDifference" id="InvAdjQtyDifference_<?php echo $index; ?>" value="<?php echo $displayDiff; ?>" readonly="readonly" style="width: 90%; height: 25px;" />
                </div>
            </td>
            <td style="width: 9%;">
                <div class="inputContainer" style="width:100%">
                    <input type="hidden" class="qtyAdjCurrent" name="total_qty[]" value="<?php echo $totalQty; ?>" />
