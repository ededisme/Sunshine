...
<div class="print_doc">
    ...
    $queryCycleProduct = mysql_query("  SELECT *,
                                        (SELECT photo FROM companies WHERE id = cycle_products.company_id) AS company_name,
                                        (SELECT name FROM location_groups WHERE id=cycle_products.location_group_id) AS location_group_name
                                        FROM cycle_products WHERE id=" . $id);
    $dataCycleProduct = mysql_fetch_array($queryCycleProduct);
    $msg = MENU_PRODUCT_INVENTORY_ADJUSTMENT;
    echo $this->element('/print/header', array('msg' => $msg, 'barcode' => $dataCycleProduct['reference'], 'address' => '', 'counterNumber' => '', 'logo' => $dataCycleProduct['company_name']));
    ?>
    ...
    <table width="100%">
        <tr>
            ...
            <td align="right" style="vertical-align: top; font-size: 10px;">
                <?php echo TABLE_ADJ_NO ?>: <b><?php echo $dataCycleProduct['reference']; ?></b>
            </td>
        </tr>
        <tr>
        ...
        <div>
            <table class="table_print">
                <tr>
                    ...
                    <th style="width: 120px !important; font-size: 10px;"><?php echo TABLE_QTY; ?></th>
                ...
                $queryDetail=mysql_query("  SELECT
                                                ...
                                                new_qty,
                                                qty_difference
                                            FROM cycle_product_details WHERE cycle_product_id=".$dataCycleProduct['id']);
                ...
                <tr>
                    ...
                    <?php
                        $value = $dataDetail['qty_difference'];
