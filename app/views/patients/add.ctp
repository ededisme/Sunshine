...
$(document).ready(function() {
    ...
    $("#PatientAddForm").ajaxForm({
        ...
        success: function(result) {
            $(".loader").attr("src",
                "<?php echo $this->webroot; ?>img/layout/spinner-placeholder.gif");
            var doctorId = $("#PatientDoctorId").val();
            if (doctorId != "") {
                $("#dialog").html(
                    '<div class="buttons"><button type="submit" class="positive printPatientNumber" ><img src="<?php echo $this->webroot; ?>img/button/print.png" alt=""/><span class="txtPrintInvoice"><?php echo ACTION_PRINT_WAITING_NUMBER; ?></span></button></div>'
                );
                $(".printPatientNumber").click(function() {
                    $.ajax({
                        type: "POST",
                        url: "<?php echo $this->base . '/' . $this->params['controller']; ?>/printDoctorWaiting/" +
                            result,
                        beforeSend: function() {
                            $(".loader").attr('src',
                                '<?php echo $this->webroot; ?>img/layout/spinner.gif'
                            );
                        },
...
<fieldset style="padding: 5px; border: 1px dashed #3C69AD;">
    ...
    <table style="margin-left: 10px; width: 97%; border-spacing: 0em 0.2em; border-collapse: separate;" cellspacing="2"
        cellpadding="2">
        <tr>
        ...
        <tr>
            ...
            <td><?php echo $this->Form->input('sex', array('empty' => SELECT_OPTION, 'label' => false, 'class' => 'validate[required]', 'style' => 'width: 200px;')); ?>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            <td><label for="PatientTelephone"><?php echo TABLE_TELEPHONE; ?>: </label> <span class="red">*</span>
                :</label></td>
