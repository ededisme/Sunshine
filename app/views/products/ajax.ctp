...
if($allowViewCost){
    ...
    'p.barcode',
    'CONCAT_WS("|*|",IFNULL(uoms.name,""),IFNULL(p.period_from,""),IFNULL(p.period_to,""),IFNULL(p.created,""),IFNULL(p.price_uom_id,""),IFNULL(p.small_val_uom,1))',
    'p.chemical',
...
/* DB table to use */
if ($category != 'all') {
    $sTable = "products p INNER JOIN product_branches ON product_branches.product_id = p.id LEFT JOIN uoms ON uoms.id = p.price_uom_id LEFT JOIN inventories ON inventories.product_id = p.id AND inventories.location_group_id IN (SELECT location_group_id FROM user_location_groups WHERE user_id = ".$user['User']['id'].") INNER JOIN product_pgroups pg ON p.id = pg.product_id AND pg.pgroup_id = " . $category;
} else {
     $sTable = "products p INNER JOIN product_branches ON product_branches.product_id = p.id LEFT JOIN uoms ON uoms.id = p.price_uom_id LEFT JOIN inventories ON inventories.product_id = p.id AND inventories.location_group_id IN (SELECT location_group_id FROM user_location_groups WHERE user_id = ".$user['User']['id'].")";
}
...
if ($_GET['sSearch'] != "") {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        if($aColumns[$i] == 'p.barcode' || $aColumns[$i] == 'p.code' || $aColumns[$i] == 'p.name' || $aColumns[$i] == 'p.chemical' || $aColumns[$i] == 'p.unit_cost' || $aColumns[$i] == 'uoms.name'){
...
while ($aRow = mysql_fetch_array($rResult)) {
    ...
    for ($i = 0; $i < count($aColumns) - 1; $i++) {
        if ($i == 0) {
            ...
            $row[] = '<a href="#" class="viewInventoryProduct" data="'.$aRow[0].'">'.displayQtyByUoM($aRow[$i], $explodeStr[4], $explodeStr[5], $explodeStr[0]).'</a>';
    ...
    $row[] =
            ...
            ($allowView ? ' <a href="" class="btnViewProductView" rel="' . $aRow[0] . '" name="' . $aRow[3] . ' - ' . $aRow[1] . '"><img alt="View" onmouseover="Tip(\'' . ACTION_VIEW . '\')" src="' . $this->webroot . 'img/button/view.png" /></a> ' : '') .
            ...
            ($allowEdit ? ' <a href="" class="btnEditProductView" rel="' . $aRow[0] . '" name="' . $aRow[3] . ' - ' . $aRow[1] . '"><img alt="Edit" onmouseover="Tip(\'' . ACTION_EDIT . '\')" src="' . $this->webroot . 'img/button/edit.png" /></a> ' : '') .
            ($allowDelete && $totalStock == 0 ? ' <a href="" class="btnDeleteProductView" rel="' . $aRow[0] . '" name="' . $aRow[3] . ' - ' . $aRow[1] . '"><img alt="Delete" onmouseover="Tip(\'' . ACTION_DELETE . '\')" src="' . $this->webroot . 'img/button/delete.png" /></a>' : '');
    $output['aaData'][] = $row;
    $lenght++;
}
