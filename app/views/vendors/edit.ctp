...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        ?>
        $("#VendorEditForm").validationEngine('attach', {
            isOverflown: true,
            overflownDIV: ".ui-tabs-panel"
        });
        $("#VendorEditForm").ajaxForm({
            beforeSerialize: function($form, options) {
                $(".btnSaveVendor").attr("disabled", "disabled");
            },
            ...
            success: function(result) {
                ...
                if(result != '<?php echo MESSAGE_DATA_HAS_BEEN_SAVED; ?>' && result != '<?php echo MESSAGE_DATA_COULD_NOT_BE_SAVED; ?>'){
                    createSysAct('Vendor', 'Edit', 2, result);
                    $("#dialog").html('<p><span class="ui-icon ui-icon-info" style="float:left; margin:0 7px 20px 0;"></span><?php echo MESSAGE_PROBLEM; ?></p>');
                }else {
                    createSysAct('Vendor', 'Edit', 1, '');
                    // alert message
...
<fieldset>
    ...
    <div style="width: 49%; vertical-align: top; float: left;">
        <?php echo $this->Form->create('Vendor', array('inputDefaults' => array('div' => false, 'label' => false)));
        ...
        ?>
        <input type="hidden" name="data[Vendor][id]" value="<?php echo $this->data['Vendor']['id']; ?>" />
        <input type="hidden" name="data[Vendor][sys_code]" value="<?php echo $this->data['Vendor']['sys_code']; ?>" />
        <input type="hidden" id="<?php echo $photoNameHidden; ?>" name="data[Vendor][new_photo]" />
        <input type="hidden" name="data[Vendor][old_photo]" value="<?php echo $this->data['Vendor']['photo']; ?>" />
        <table style="width: 100%;">
            <tr>
                <td><label for="VendorVendorCode"><?php echo TABLE_VENDOR_NUMBER; ?> <span class="red">*</span> :</label></td>
                <td>
                    <div class="inputContainer">
                        <?php echo $this->Form->text('vendor_code', array('class' => 'validate[required]')); ?>
                    </div>
                </td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td>
                    <input type="hidden" name="data[Vendor][note]" value="<?php echo $this->data['Vendor']['note']; ?>" />
                    <div class="inputContainer">
                        <?php echo $this->Form->input('vgroup_id', array('selected' => $vgroupsSellected, 'label' => false, 'empty' => INPUT_SELECT)); ?>
                    </div>
                </td>
            </tr>
            <tr>
    ...
    <div style="vertical-align: top; float: right; width: 49%;">
        ...
        <table style="width: 100%">
            ...
            <tr>
                ...
                <td>
                    <div class="inputContainer">
                        <textarea id="VendorContactNote"><?php echo $this->data['Vendor']['note']; ?></textarea>
                    </div>
                </td>
            </tr>
        </table>
