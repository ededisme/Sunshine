 ...
 */
$date = date('Y-m-d');
$tableName = "general_ledger_detail_ven" . $user['User']['id'];
...
$queryCoa = mysql_query("   SELECT SUM(debit),SUM(credit),chart_account_id,company_id,location_id,customer_id,vendor_id,employee_id
                            ...
                            WHERE gl.is_approve=1 AND gl.is_active=1 AND date <= '" . $date . "' AND chart_account_id IN (" . implode(",", $arrCoAIdList) . ") AND company_id IN (SELECT company_id FROM user_companies WHERE user_id = ".$user['User']['id'].") AND vendor_id IS NOT NULL AND vendor_id!=''
 ...
 */
$aColumns = array('id', 'vendor_code', 'name', 'work_telephone', 'other_number', 'fax_number', 'email_address', 'FORMAT((SELECT SUM(debit)-SUM(credit) AS amount FROM ' . $tableName . ' WHERE chart_account_id IN (' . implode(",", $arrCoAIdList) . ') AND company_id IN (SELECT company_id FROM user_companies WHERE user_id = '.$user['User']['id'].') AND vendor_id=vendors.id)*-1,2)');
...
while ($aRow = mysql_fetch_array($rResult)) {
    ...
    $row[] =
            ($allowView ? '<a href="" class="btnViewVendor" rel="' . $aRow[0] . '" name="' . $aRow[2] . '"><img alt="View" onmouseover="Tip(\'' . ACTION_VIEW . '\')" src="' . $this->webroot . 'img/button/view.png" /></a> ' : '') .
            ($allowEdit ? '<a href="" class="btnEditVendor" rel="' . $aRow[0] . '" name="' . $aRow[2] . '"><img alt="Edit" onmouseover="Tip(\'' . ACTION_EDIT . '\')" src="' . $this->webroot . 'img/button/edit.png" /></a> ' : '') .
            ($allowDelete ? '<a href="" class="btnDeleteVendor" rel="' . $aRow[0] . '" name="' . $aRow[2] . '"><img alt="Delete" onmouseover="Tip(\'' . ACTION_DELETE . '\')" src="' . $this->webroot . 'img/button/delete.png" /></a>' : '');
    $output['aaData'][] = $row;
}
