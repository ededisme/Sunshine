...
$sqlShare = mysql_query("SELECT * FROM user_shares WHERE module_type_id = 68 AND is_active = 1 AND user_id = ".$user['User']['id']." ORDER BY id DESC LIMIT 1");
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        loadOrderDetailQuotation();
        loadAutoCompleteOff();
        // Hide Branch
        $("#QuotationBranchId").filterOptions('com', '0', '');
        $("#QuotationAddForm").validationEngine();
        ...
        $("#QuotationAddForm").ajaxForm({
            ...
            error: function (result) {
                ...
                createSysAct('Quotation', 'Add', 2, result.responseText);
        ...
        $("#QuotationCustomerName").autocomplete("<?php echo $this->base ."/".$this->params['controller']. "/searchCustomer"; ?>", {
            ...
            formatItem: function(data, i, n, value) {
                if(checkCompanyQuote(value.toString().split(".*")[3])){
        ...
        // Company Action
        if($.cookie('companyIdQuotation')!=null || $("#QuotationCompanyId").find("option:selected").val() != ''){
            if($.cookie('companyIdQuotation') != null){
                $("#QuotationCompanyId").val($.cookie('companyIdQuotation'));
            }
            var vatCal = $("#QuotationCompanyId").find("option:selected").attr("vat-opt");
            $("#QuotationVatCalculate").val(vatCal);
            $("#QuotationBranchId").filterOptions('com', $("#QuotationCompanyId").val(), '');
            $("#QuotationBranchId").change();
        }
        // User Share
    ...
    function checkVatCompanyQuotation(){
        // VAT Filter
        $("#QuotationVatSettingId").filterOptions('com-id', $("#QuotationCompanyId").val(), '');
    }
    ...
    function loadOrderDetailQuotation(){
        $.ajax({
            type: "POST",
    ...
    function changeInputCSSQuotation(){
        ...
        // Label
        $("#QuotationAddForm").find("label").removeAttr("class");
        $("#QuotationAddForm").find("label").each(function(){
        ...
        // Input & Select
        $("#QuotationAddForm").find("input").each(function(){
            $(this).removeClass(cssRemove);
            $(this).addClass(cssStyle);
        });
        $("#QuotationAddForm").find("select").each(function(){
...
<?php 
echo $this->Form->create('Quotation', array('inputDefaults' => array('div' => false, 'label' => false))); 
if(!empty($rowShare['share_option'])){
    $shareOpt = $rowShare['share_option'];
} else {
    $shareOpt = 1;
}
?>
<input type="hidden" value="" name="data[Quotation][vat_calculate]" id="QuotationVatCalculate" />
<input type="hidden" value="" name="data[Quotation][currency_center_id]" id="QuotationCurrencyCenterId" />
<input type="hidden" value="1" name="data[Quotation][share_save_option]" id="QuotationShareSaveOption" />
<input type="hidden" value="" name="data[Quotation][user_share_id]" id="QuotationUserShareId" />
<input type="hidden" value="<?php echo $shareOpt; ?>" name="data[Quotation][share_option]" id="QuotationShareOption" />
<input type="hidden" value="<?php echo $rowShare['share_users']; ?>" name="data[Quotation][share_user]" id="QuotationShareUser" />
<input type="hidden" value="<?php echo $rowShare['share_except_users']; ?>" name="data[Quotation][share_except_user]" id="QuotationShareExceptUser" />
                                ...
                                <tr>
                                    <td>
                                        <div class="inputContainer" style="width:100%">
                                            ...
                                            <?php echo $this->Form->text('customer_name', array('label' => false, 'class' => 'validate[required,funcCall[checkCustomerQuotation]]', 'style' => 'width:80%')); ?>
                                            <img alt="Search" align="absmiddle" style="cursor: pointer; width: 22px; height: 22px;" class="searchCustomerQuotation" onmouseover="Tip('<?php echo GENERAL_SEARCH; ?>')" src="<?php echo $this->webroot . 'img/button/search.png'; ?>" />
                                            <img alt="Delete" align="absmiddle" style="cursor: pointer; display: none;" class="deleteCustomerQuotation" onmouseover="Tip('<?php echo ACTION_DELETE; ?>')" src="<?php echo $this->webroot . 'img/button/delete.png'; ?>" />
                                        </div>
                                    </td>
                                    <td>
                                        <div class="inputContainer" style="width:100%">
                                    ...
                                    <td>
                                        <div class="inputContainer" style="width:100%">
                                            <?php echo $this->Form->text('quotation_date', array('value' => date("d/m/Y"),'readonly' => 'readonly', 'class' => 'validate[required]', 'style' => 'width:85%')); ?>
                                        </div>
                                    </td>
                                </tr>
                            </table>
    ...
    <div style="float: right; width:78%;">
        <table style="width: 100%;">
            <tr>
                ...
                <td style="width: 13%;">
                    <div class="inputContainer" style="width: 100%">
                        <?php echo $this->Form->text('total_amount', array('readonly' => true, 'class' => 'float validate[required]', 'style' => 'width: 80%; font-size:12px; font-weight: bold', 'value'=>'0')); ?> <span class="lblSymbolQuotation"></span>
                    </div>
                </td>
                ...
                <td style="width: 20%;">
                    <div class="inputContainer" style="width:100%">
                        ...
                        <?php echo $this->Form->text('discount', array('style' => 'width: 55%; height:15px; font-size:12px; font-weight: bold', 'class' => 'float', 'value'=>'0', 'readonly' => true)); ?> <span class="lblSymbolQuotation"></span>
                        <span id="quoteLabelDisPercent"></span>
                        <?php if($allowEditInvDis){ ?><img alt="Remove" src="<?php echo $this->webroot . 'img/button/cross.png'; ?>" id="btnRemoveQuotationTotalDiscount" align="absmiddle" style="cursor: pointer; display: none;" onmouseover="Tip('Remove Discount')" /><?php } ?>
                    </div>
                </td>
                ...
                <td style="width: 9%;">
                    <div class="inputContainer" style="width: 100%">
                        <input type="hidden" value="0" name="data[Quotation][total_vat]" id="QuotationTotalVat" class="float" />
                        <?php echo $this->Form->text('vat_percent', array('readonly' => true, 'class' => 'float validate[required]', 'style' => 'width: 50%; font-size:12px; font-weight: bold', 'value' => 0)); ?> <span class="lblSymbolQuotationPercent">(%)</span>
                    </div>
                </td>
                <td style="width: 6%; text-align: right;"><label for="QuotationTotalAmountSummary"><?php echo TABLE_TOTAL; ?>:</label></td>
                <td style="width: 14%;">
                    <div class="inputContainer" style="width: 100%">
                        <?php echo $this->Form->text('total_amount_summary', array('readonly' => true, 'class' => 'float validate[required]', 'style' => 'width: 80%; font-size:12px; font-weight: bold', 'value'=>'0')); ?> <span class="lblSymbolQuotation"></span>
                    </div>
                </td>
            </tr>
        </table>
