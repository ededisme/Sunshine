 ...
 */
$aColumns = array("CONCAT(p.id,'!-',p.price_uom_id,'!-',IF((SELECT count(*) FROM `purchase_order_details` WHERE product_id = p.id) > 0, (SELECT qty_uom_id FROM `purchase_order_details` WHERE product_id = p.id ORDER BY `id` DESC LIMIT 1), p.price_uom_id))", 
                  'p.code', 
                  "p.name", 
...
/* Customize condition */
$condition = "p.is_active = 1 AND p.is_packet = 0 AND p.price_uom_id > 0 AND p.small_val_uom > 0";
if (@$companyId) {
...
while ($aRow = mysql_fetch_array($rResult)) {
    ...
    $smallUom = 1;
    $smallUomLabel = "";
    $qry = mysql_query("SELECT value, (SELECT abbr FROM uoms WHERE id = uom_conversions.to_uom_id) as abbr FROM uom_conversions WHERE from_uom_id = " . $array[1] . " AND is_small_uom = 1 AND is_active = 1");
    while (@$d = mysql_fetch_array($qry)) {
        $smallUom = $d['value'];
        $smallUomLabel = $d['abbr'];
    }
    $unit_cost = $aRow[3];
    $proName   = str_replace('"', '&quot;', $aRow[2]);
    $data = array();
    $data[] = trim($array[0]);
    $data[] = trim($array[1]);
    $data[] = trim($unit_cost);
    $data[] = trim(htmlspecialchars($aRow[1], ENT_QUOTES, 'UTF-8'));
    $data[] = trim(htmlspecialchars($proName, ENT_QUOTES, 'UTF-8'));
    $data[] = trim($smallUom);
    for ($i = 0; $i < count($aColumns); $i++) {
        if ($i == 0) {
            /* Special output formatting */
            $index++;
            $row[] = "<input type='radio' value='".json_encode($data)."' id='".trim(htmlspecialchars($proName, ENT_QUOTES, 'UTF-8'))."' class='".htmlspecialchars(trim($aRow[1]), ENT_QUOTES, 'UTF-8')."' name='chkProduct' />";
        } else if ($i == 3) {
