...
$allowPrint = checkAccess($user['User']['id'], $this->params['controller'], 'printPatientService');
$allowAdd = checkAccess($user['User']['id'], $this->params['controller'], 'patientPayment');
...
while ($aRow = mysql_fetch_array($rResult)) {
    ...
    $row[] = ($allowAdd ? '<a href="" class="btnPayment" rel="' . $aRow[$aColumns[0]] . '" title="' . $aRow[$aColumns[1]] . '"><img alt="Aging" onmouseover="Tip(\'' . TABLE_PAY . '\')" src="' . $this->webroot . 'img/button/pay_24.png" /></a>' : '');       
