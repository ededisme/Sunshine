...
<div id="print-content" class="print_doc">
   <?php
    ...
    <table class="table" cellspacing="0" style="padding-bottom: 100px" >
        ...
        ?>
        <tr>
            <td class="first" colspan="7"><?php echo SERVICE_SERVICE; ?></td>
        </tr>
        <?php
        $query=mysql_query("SELECT invoice_details.total_price,invoice_details.doctor_id,invoice_details.qty,invoice_details.unit_price,invoice_details.discount,ser.name as serviceName FROM invoice_details INNER JOIN services ser ON ser.id=invoice_details.service_id INNER JOIN sections sec ON ser.section_id=sec.id WHERE type=1 AND invoice_details.is_active=1 AND invoice_id=".$this->params['pass'][0]);
        ...
        <tr>
            ...
            <td style="width:30%;"><?php echo $data['serviceName'];?></td>
            <?php
            $doctorName = "";
            if($data['doctor_id']!=""){
        ...
        <?php } ?>
         <tr>
            <td class="first" colspan="7"><?php echo MENU_LABO_MANAGEMENT; ?></td>
        </tr>
        <?php
        $query=mysql_query("SELECT invoice_details.total_price,invoice_details.doctor_id,invoice_details.qty,invoice_details.unit_price,invoice_details.discount,labo_item_groups.name as Name FROM invoice_details INNER JOIN labo_item_groups ON labo_item_groups.id=invoice_details.service_id WHERE type=2 AND invoice_details.is_active=1 AND invoice_id=".$this->params['pass'][0]);
        ...
        <tr>
            ...
            ?>
            <td class="first"><?php echo $index++; ?></td> 
            <td style="width:30%;"><?php echo $data['Name'];?></td>
            <?php
            $doctorName = "";
            if($data['doctor_id']!=""){
               $doctorName = getDoctor($data['doctor_id']);               
        ...
        <tr>
            <td class="first" colspan="7"><?php echo 'Medicine'; ?></td>
        </tr>
        <?php                
        $query=mysql_query("SELECT sales_orders.total_amount,sales_orders.discount,products.name,sales_order_details.qty,sales_order_details.unit_price,sales_order_details.total_price,sales_order_details.discount_amount,orders.created_by FROM sales_orders LEFT JOIN orders ON orders.id = sales_orders.order_id INNER JOIN sales_order_details ON sales_orders.id=sales_order_details.sales_order_id INNER JOIN products ON products.id=sales_order_details.product_id WHERE sales_orders.status>=1 AND sales_orders.queue_id =".$patient['Invoice']['queue_id']);
        while($data=mysql_fetch_array($query)){?>
        <tr>
            <?php
            $total += ($data['total_price']-$data['discount_amount']);
            ?>
            <td class="first"><?php echo $index++; ?></td>    
            <td style="width:30%;"><?php echo $data['name']; ?></td>
            <?php
            ...
            <td style="text-align: center;"><?php echo $data['qty']; ?></td>
            <td style="text-align: right;"><?php echo number_format($data['unit_price'], 2); ?></td>
            <td style="text-align: right;"><?php echo number_format($data['discount_amount'], 2); ?></td>
            <td style="text-align: right;"><?php echo number_format($data['total_price']-$data['discount_amount'], 2); ?></td>
        </tr>
        <?php } ?>
        <tr>
