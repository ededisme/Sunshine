...
if ($customer != "all") {
    $condition .= " AND a.patient_id = " . $customer;
}

if ($dateFrom != "") {
    $condition .= " AND DATE( a.app_date ) >= '" . $dateFrom . "'";
}
if($dateTo){
    $condition .= " AND DATE( a.app_date ) <= '" . $dateTo . "'";
} 
if ($dateFrom == "" && $dateTo == "") {
...
while ($aRow = mysql_fetch_array($rResult)) {
    ...
    if (!mysql_num_rows($query_consult)) {  
        $returns = ($allowReturnPatient ? '<a href="" class="btnReturnAppointment" doctor-id="'.$aRow[7].'" rel="' . $explodeStr[1] . '" title="' . PATIENT_ADD_TO_QUEUE .': '. $aRow[1] .' - '. $aRow[2] . '"><img style="width:16px;" alt="Return" onmouseover="Tip(\'' . ACTION_RETURN . '\')" src="' . $this->webroot . 'img/action/return.png" /></a>&nbsp;&nbsp;' : '');
    } 
