...
<script type="text/javascript">
    $(document).ready(function(){
        // Prevent Key Enter
        preventKeyEnter();
        $("#ChartAccountGroupEditForm").validationEngine();
        $("#ChartAccountGroupEditForm").ajaxForm({
            ...
            success: function(result) {
                ...
                if(result != '<?php echo MESSAGE_DATA_HAS_BEEN_SAVED; ?>' && result != '<?php echo MESSAGE_DATA_COULD_NOT_BE_SAVED; ?>'){
                    createSysAct('Chart Account Group', 'Edit', 2, result);
                    $("#dialog").html('<p><span class="ui-icon ui-icon-info" style="float:left; margin:0 7px 20px 0;"></span><?php echo MESSAGE_PROBLEM; ?></p>');
                }else {
                    createSysAct('Chart Account Group', 'Edit', 1, '');
                    // alert message
        ...
        <?php if($this->data['ChartAccountGroup']['chart_account_type_id']!=13){ ?>
...
<br />
<?php 
echo $this->Form->create('ChartAccountGroup'); 
