...
$allowAdd=checkAccess($user['User']['id'], $this->params['controller'], 'addMidWifeServiceDoctor');
?>
<?php $tblName = "tbl" . rand(); ?>
<script type="text/javascript" src="<?php echo $this->webroot; ?>js/pipeline.js"></script>
<script type="text/javascript">
    var oTableMidWifeService;
    $(document).ready(function(){
        // Prevent Key Enter
        preventKeyEnter();
        $("#example td:first-child").addClass('first');
        oTableMidWifeService = $("#example").dataTable({
            "bProcessing": true,
            "bServerSide": true,
            "sAjaxSource": "<?php echo $this->base.'/'.$this->params['controller']; ?>/echoServiceDoctorAjax/",
            ...
            "fnInfoCallback": function( oSettings, iStart, iEnd, iMax, iTotal, sPre ) {
                ...
                $(".btnMidWifeServiceDoctor").click(function(event){
                    ...
                    rightPanel.load("<?php echo $this->base; ?>/<?php echo $this->params['controller']; ?>/addMidWifeServiceDoctor/" + queueDoctorId+"/"+queueId);
                });
                return sPre;
            },
            "aoColumnDefs": [{
