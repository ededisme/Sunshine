    ...
    <table style="width: 100%; color: #083181;" cellpadding="5" cellspacing="0">
        ...
        <tr>
            ...
            <td style="float: right;">
                <?php
                echo '<a class="btnPrintSticker" pnt-name="'.$resultPatient['patient_name'].'" doctorName="'.$doctorName.'" weight="'.$resultPatientVital['weight'].'" sex="'.$gender.'" dob="'.$dob.'" rel="' . $orderDetail['OrderDetail']['id'] . '" pro-name="' . $productName . '" qty="' . $orderDetail['OrderDetail']['qty'] . '" uom="' . $orderDetail['Uom']['abbr'] . '" note="' . $orderDetail['OrderDetail']['note'] . '" frequency="' . $medicinUseMorning . '" num-day="' . $orderDetail['OrderDetail']['num_days'] . '" ><img alt="Print Sticker" onmouseover="Tip(\'' . 'Print Sticker' . '\')" src="' . $this->webroot . 'img/button/printer.png" /></a> &nbsp;&nbsp;&nbsp;';
                ?>
            </td>
        </tr>
        <?php } ?>
